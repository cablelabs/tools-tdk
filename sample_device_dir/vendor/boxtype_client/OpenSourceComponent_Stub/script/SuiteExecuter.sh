...
'
   ...
   if [[ "$COMP_NAME" == "jansson" || "$COMP_NAME" == "yajl" ]]; then
	...
	cat  $LOG_PATH/log_$COMP_NAME|grep "$KEY_WORD" > $EXECUTER_PATH/SUITE_STATUS
	   ...
	   echo "Test binaries are not present in " $BIN_PATH
           echo "Test binaries are not present in " $BIN_PATH > $EXECUTER_PATH/SUITE_STATUS
           echo "Test binaries are not present in " $BIN_PATH > $EXECUTER_PATH/LOGPATH_INFO
           exit 1
        fi
