...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1271</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_FontSize_BoundHigh_60</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of font size- Higher boundary value.		
CT_ClosedCaption_60</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_60</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of font size- Higher boundary value.</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 6 (list value of font size in the structure)(higher value), ccType - Digital/Analog.</input_parameters>
    <automation_approch>1. TM loads the ClosedCaption_Manager_Agent via the test agent.
2.ClosedCaption_Manager_Agent will initialize/start the ClosedCaption Manager.                        3.ClosedCaption_Manager_Agent will set the font size of the ClosedCaption Manager.                                                    4. ClosedCaption Manager component will return the font size through the ClosedCaption_Manager_Agent to the TM.                                                                          5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_FontSize_BoundHigh_60</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_FontSize_BoundHigh_60');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    #calling closed caption API "CC_SetGetAttribute" to get the attribute (Font bound high) of the closed caption
    tdkTestObj = obj.createTestStep('CC_SetGetAttribute');
    categories = "size";
    print "Categories value set :%s" %categories;
    attrib_type = 32; # To set Fontsize high bound
    print "Attrib value set :%d" %attrib_type;
    font_size = 2; # High bound
    print "Font size value set :%d" %font_size;
    ...
    attrib_type = "2" ;
    print "Expected default font size bound high is: 2";
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      print "Get Font size bound high %s" %cc_Get_Attribute_details;
      if attrib_type in cc_Get_Attribute_details:
        tdkTestObj.setResultStatus("SUCCESS");
        print "-- Value of font size bound high is as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Value of font size bound high not as expected --";
        print "FAILURE: with %s " %tdkTestObj.getResultDetails();

    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE:Font size bound high attribute is not returned";
