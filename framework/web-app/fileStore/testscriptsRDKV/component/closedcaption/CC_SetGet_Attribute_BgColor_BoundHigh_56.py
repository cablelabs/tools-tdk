...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1268</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_BgColor_BoundHigh_56</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of background color - Higher boundary value.	
Test case Id - CT_ClosedCaption_56</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_56</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of background color - Higher boundary value.</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list,         type - 0 (list value of charBgColor in the structure)(higher value) BgColor,                             ccType - Digital/Analog.</input_parameters>
...
2.ClosedCaption_Manager_Agent will initialize/start the ClosedCaption Manager.                        3.ClosedCaption_Manager_Agent will set the back ground color of the ClosedCaption Manager.                           4. ClosedCaption Manager will return the back ground color through the ClosedCaption_Manager_Agent to the TM.                                                                            5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_BgColor_BoundHigh_56</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_BgColor_BoundHigh_56');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    color_type = 7;
    print "Bg color bound high value set to: %d" %color_type
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      print "get Bg color bound high %s" %cc_Get_Attribute_details;
      ...
      if flag == 0:
        ...
        print "-- Get and set values of bg color bound high are same --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Get and set values of bg color bound high are not same --";
       
    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Bg color bound high attribute is not set with %s" %tdkTestObj.getResultDetails();
