...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1273</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_FontUnderline_BoundLow_76</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of font underline- Lower boundary value.
Test case Id - CT_ClosedCaption_76</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_76</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of font underline- Lower boundary value.</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 9 (list value of fontUnderline in the structure)(lower value), ccType - Digital/Analog.</input_parameters>
    <automation_approch>1. TM loads the ClosedCaption_Manager_Agent via the test agent.
2.ClosedCaption_Manager_Agent will initialize/start the  ClosedCaption Manager.                        3.ClosedCaption_Manager_Agent will set the underlined font to the ClosedCaption Manager.                                                    4. ClosedCaption Manager will return the underlined font through the ClosedCaption_Manager_Agent to the TM.                                                                          5. ClosedCaption_Manager_Agent will return SUCCESS status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_FontUnderline_BoundLow_76</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_FontUnderline_BoundLow_76');
...
#calling closed caption API "CC_SetGetAttribute" to set the attribute (FontUnderline) of the closed caption
    ...
    fontUnderline = 0;
    print "Font underline value set to: %d" %fontUnderline
    ...
    print "Set font underline low bound %s" %fontUnderline;
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      ...
      if attrib_type in cc_Get_Attribute_details:
        ...
        print "-- Get and set values of low bound font underline are same --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Get and set values of low bound font underline are not same --";
        print "FAILURE:with %s" %tdkTestObj.getValue("details");

    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: font underline low bound attribute is not set with %s" %tdkTestObj.getValue("details");
