...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1276</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_FontItalic_BoundHigh_61</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set and get the  closed caption attribute of font Italic (Italicized font)- Higher boundary value.
Test case Id - CT_ClosedCaption_61</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_61</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of font Italic (Italicized font)- Higher boundary value.</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 8 (list value of fontItalic in the structure)(higher value), ccType - Digital/Analog.</input_parameters>
...
2.ClosedCaption_Manager_Agent will initialize/start the closed caption manager with ClosedCaption Manager component.                        3.ClosedCaption_Manager_Agent will set the Italicized font to the closed caption manager with ClosedCaption Manager component.                                                    4. ClosedCaption Manager component will return the Italicized font through the ClosedCaption_Manager_Agent to the TM.                                                                          5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_FontItalic_BoundHigh_61</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_FontItalic_BoundHigh_61');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    #calling closed caption API "CC_SetGetAttribute" to get the attribute (FontItalic bound high) of the closed caption
    ...
    print "Attrib value set to :%s" %attrib_type;
    fontItalic = 1; #
    print "Font Italic set to :%s" %fontItalic;
    ...
    attrib_type = "1";
    print "Expected  font Italic bound high is: 1";
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      print "Get Font Italic bound high %s" %cc_Get_Attribute_details;
      if attrib_type in cc_Get_Attribute_details:
        tdkTestObj.setResultStatus("SUCCESS");
        print "-- Value of Font Italic bound high is as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Value of Font Italic bound high is not as expected --";
        print "FAILURE: with %s " %tdkTestObj.getResultDetails();

    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Font Italic bound high attribute is not returned";
