...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1269</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_FontOpacity_BoundHigh_57</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of foreground opacity- Higher boundary value .
Tese case Id - CT_ClosedCaption_57</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_57</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of foreground opacity- Higher boundary value .</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 4 (list value of charFgOpacity in the structure)(higher value), ccType - Digital/Analog.</input_parameters>
    <automation_approch>1. TM loads the ClosedCaption_Manager_Agent via the test agent.
2.ClosedCaption_Manager_Agent will initialize/start the closed caption manager with ClosedCaption Manager component.                        3.ClosedCaption_Manager_Agent will set the foreground opacity of the closed caption manager with ClosedCaption Manager component.                           4. ClosedCaption Manager component will return the foreground opacity through the ClosedCaption_Manager_Agent to the TM.                  5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_FontOpacity_BoundHigh_57</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_FontOpacity_BoundHigh_57');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      print "Get Font opacity bound high %s" %cc_Get_Attribute_details;
      if attrib_type in cc_Get_Attribute_details:
        tdkTestObj.setResultStatus("SUCCESS");
        print "-- Font opacity bound high is as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Font opacity bound high is not as expected --";
        print "FAILURE: with %s " %tdkTestObj.getResultDetails(); 

    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Font opacity bound high attribute is not returned";
