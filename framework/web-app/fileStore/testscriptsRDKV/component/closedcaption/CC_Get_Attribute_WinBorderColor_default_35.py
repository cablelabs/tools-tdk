...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1252</id>
  <version>1</version>
  <name>CC_Get_Attribute_WinBorderColor_default_35</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption -To check the default  attribute value of window border color.			
Test case Id - CT_ClosedCaption_35</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_35</test_case_id>
    <test_objective>Closed Caption -To check the default  attribute value of window border color .</test_objective>
    ...
    <input_parameters>Categories : CCGetAttribute : *attrib - Pointer to cc attribute structure list, type - 2 (list value of winColor in the structure), ccType - Digital/Analog.</input_parameters>
...
5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_Get_Attribute_WinBorderColor_default_35</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_Get_Attribute_WinBorderColor_default_35');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    border_type = 1001; # default win border color
    ...
    print "Expected default Win Border color is:  %s" %iterator[0];
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      ...
      if iterator[0] in retattribute:
        flag = 0;             
      else:
        flag =1;
      if flag == 0:
        tdkTestObj.setResultStatus("SUCCESS");
        print "-- default value of Win border color is as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- default value of Win border color is not as expected --";
       
    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Did not return the default Win border color attribute" ;
