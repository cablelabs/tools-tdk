...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1274</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_BorderType_BoundHigh_63</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of border type (window border type)- Higher boundary value.			
Test case Id - CT_ClosedCaption_63</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_63</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of border type (window border type)- Higher boundary value.</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 10 (list value of window border type the structure)(higher value), ccType - Digital/Analog.</input_parameters>
...
2.ClosedCaption_Manager_Agent will initialize/start the  ClosedCaption Manager.                        3.ClosedCaption_Manager_Agent will set the border color to the ClosedCaption Manager.                                                    4. ClosedCaption Manager will return the border color through the ClosedCaption_Manager_Agent to the TM.                                                                          5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_BorderType_BoundHigh_63</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_BorderType_BoundHigh_63');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    #calling closed caption API "CC_SetGetAttribute" to get the attribute (Border type bound high) of the closed caption
    ...
    border_type = 5;
    print "Border type bound high value set to:%d" %border_type
    ...
    attrib_type = "5";
    print "Expected default border type bound high : 5";
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      print "Get border type bound high %s" %cc_Get_Attribute_details;
      if attrib_type in cc_Get_Attribute_details:
        tdkTestObj.setResultStatus("SUCCESS");
        print "-- Value of border type bound high is as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Value of border type bound high is not as expected --";
        print "FAILURE: with %s " %tdkTestObj.getResultDetails();

    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Border type bound high attribute is not returned";
