...
<xml>
  ...
  <name>CC_SetGet_Attribute_WindowOpacity_BoundHigh_66</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Set and get the  closed caption attribute of window opacity - Higher boundary value.
Test case Id - CT_ClosedCaption_66</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_WinOpacity_BoundHigh_66')
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    #calling closed caption API "CC_SetGetAttribute" to get the attribute (Win Opacity bound high) of the closed caption
    ...
    win_opacity = 3;
    print "Win_opacity bound high value set to : %d" %win_opacity
    ...
    attrib_type = "3";
    print "Set Win opacity bound high value : 3"
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      print "Get Win opacity bound high %s" %cc_Get_Attribute_details;
      if attrib_type in cc_Get_Attribute_details:
        tdkTestObj.setResultStatus("SUCCESS");
        print "-- The value of Win opacity bound high is as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- The value of Win opacity bound high is not as expected --";
        print "FAILURE: with %s " %tdkTestObj.getResultDetails();

    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Win opacity bound high attribute is not returned";
