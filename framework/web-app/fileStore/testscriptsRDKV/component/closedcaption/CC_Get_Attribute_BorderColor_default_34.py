...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1251</id>
  <version>1</version>
  <name>CC_Get_Attribute_BorderColor_default_34</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - To check the default  attribute value of border color (border color).	
Test case Id - CT_ClosedCaption_34</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_34</test_case_id>
    <test_objective>Closed Caption - To check the default  attribute value of border color (border color).</test_objective>
    ...
    <input_parameters>Categories : CCGetAttribute : *attrib - Pointer to cc attribute structure list, type - 11 (list value of window border color the structure), ccType - Digital/Analog.</input_parameters>
...
5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_Get_Attribute_BorderColor_default_34</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_Get_Attribute_BorderColor_default_34');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    border_color = 1001; # default Border color
    ...
    print "Expected default Border color is:  %s" %iterator[0];
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      ...
      if flag == 0:
        ...
        print "-- default value of border color is as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- default value of border color is not as expected --";
       
    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Did not return the default border color attribute" ;
