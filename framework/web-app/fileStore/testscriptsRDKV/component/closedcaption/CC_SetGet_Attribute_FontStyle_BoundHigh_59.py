...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1270</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_FontStyle_BoundHigh_59</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of font style- Higher boundary value.		
Test case Id - CT_ClosedCaption_59</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_59</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of font style- Higher boundary value.</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 7 (list value of font style in the structure)(higher value), ccType - Digital/Analog.</input_parameters>
...
2.ClosedCaption_Manager_Agent will initialize/start the  ClosedCaption Manager.                        3.ClosedCaption_Manager_Agent will set the font style of the ClosedCaption Manager.                                                    4. ClosedCaption Manager will return the font style through the ClosedCaption_Manager_Agent to the TM.                                                                          5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_FontStyle_BoundHigh_59</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_FontStyle_BoundHigh_59');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    font_style =  8;
    print "Font style high bound value set to: %s" %font_style
    ...
    attrib_type = "SmallCapital";
    print "Set font style bound high %s" %font_style;
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      print "Get font style bound high %s" %cc_Get_Attribute_details;
      if attrib_type in cc_Get_Attribute_details:
        tdkTestObj.setResultStatus("SUCCESS");
        print "-- Set and get values of font style bound high is as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Set and get values of font style bound high is not as expected --";
        print "FAILURE: with %s " %tdkTestObj.getResultDetails(); 

    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Font style bound high attribute is not returned";
