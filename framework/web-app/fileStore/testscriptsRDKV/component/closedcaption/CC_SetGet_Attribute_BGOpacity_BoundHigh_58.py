...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1243</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_BGOpacity_BoundHigh_58</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of background opacity - Higher boundary value.	
Test case Id - CT_ClosedCaption_58</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_58</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of background opacity - Higher boundary value.</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 4 (list value of charBgOpacity in the structure)(higher value), ccType - Digital/Analog.</input_parameters>
...
2.ClosedCaption_Manager_Agent will initialize/start the closed caption manager with ClosedCaption Manager component.                        3.ClosedCaption_Manager_Agent will set the background opacity of the closed caption manager with ClosedCaption Manager component.                           4. ClosedCaption Manager component will return the foreground opacity through the ClosedCaption_Manager_Agent to the TM.                  5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_BGOpacity_BoundHigh_58</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_BGOpacity_BoundHigh_58');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    attrib_type = 8; 
    print "attrib_type value set to: %s" %attrib_type
    opacity_type = 3; 
    print "Bg opacity bound high value set to: %s" %opacity_type 
    ...
    attrib_type = "3";
    print "Set Bg opacity bound high to :3";
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      ...
      if attrib_type in cc_Get_Attribute_details:
        ...
        print "-- Get and set values of Bg opacity bound high are same --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Get and set values of Bg opacity bound high are not same --";
        print "FAILURE: with %s " %tdkTestObj.getResultDetails(); 

    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: Bg opacity bound high attribute is not set with %s" %tdkTestObj.getResultDetails();
