...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1234</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_FontColor_BoundHigh_55</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of foreground color - Higher boundary value
Test case Id - CT_ClosedCaption_55</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_55</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of foreground color - Higher boundary value</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 1 (list value of charFgColor in the structure) (higher value), ccType - Digital/Analog.</input_parameters>
    <automation_approch>1. TM loads the ClosedCaption_Manager_Agent via the test agent.
2.ClosedCaption_Manager_Agent will initialize/start the closed caption manager with ClosedCaption Manager component.                        3.ClosedCaption_Manager_Agent will set the fore ground color of the closed caption manager with ClosedCaption Manager component.                                                 4. ClosedCaption Manager component will return the fore ground color through the ClosedCaption_Manager_Agent to the TM.                                                                 5. ClosedCaption_Manager_Agent will return SUCCESS/FAILURE status based on the return value of  APIs.</automation_approch>
    ...
    <test_script>CC_SetGet_Attribute_FontColor_BoundHigh_55</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_FontColor_BoundHigh_55');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    attrib_type = 1; # To set Font Color bound high
    print "attrib_type value set to: %d" %attrib_type
    color_type = 7; 
    print "Font color bound high value set to: %d" %color_type
    ...
    print "Set font color bound high : %s" %iterator[color_type];
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      print "get font color bound high %s" %cc_Get_Attribute_details;
      ...
      if flag == 0:
        ...
        print "-- Get and set higher values of font color are same --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Get and set higher values of font color are not same --";
       
    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE: font color high bound attribute is not set with %s" %tdkTestObj.getResultDetails();
