...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1264</id>
  <version>1</version>
  <name>CC_SetGet_Attribute_WinBorderColor_invalid_51</name>
  <primitive_test_id>194</primitive_test_id>
  <primitive_test_name>CC_SetGetAttribute</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Closed Caption - Set and get the  closed caption attribute of window border color with invalid parameter.
Test case Id - CT_ClosedCaption_51</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_ClosedCaption_51</test_case_id>
    <test_objective>Closed Caption - Set and get the  closed caption attribute of window border color with invalid parameter.</test_objective>
    ...
    <input_parameters>Categories : CCSetGetAttribute : *attrib - Pointer to cc attribute structure list, type - 2 (list value of winColor not in the structure)(invalid value), ccType - Digital/Analog.</input_parameters>
...
Checkpoint 2.Check the value which is being set is being got back by the get API.</except_output>
    ...
    <test_script>CC_SetGet_Attribute_WinBorderColor_invalid_51</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CC_SetGet_Attribute_WinBorderColor_invalid_51');
...
if "Success" in loadmodulestatus:
  ...
  if "SUCCESS" in loadmodulestatus.upper():
    ...
    #calling closed caption API "CC_SetGetAttribute" to set the attribute (Win Border Color Invalid) of the closed caption
    ...
    border_type = 1000; # Invalid win border color
    #print "Win Border color value set to: %d" %border_type
    ...
    print "Set Win Border color : invalid Win Border color 123456789";
    ...
    if cc_Set_Attribute_expectedresult in cc_Set_Attribute_actualresult:
      ...
      if flag == 0:
        ...
        print "-- Get and set values invalid Win border color are not same as expected --";
      else:
        tdkTestObj.setResultStatus("FAILURE");
        print "-- Get and set values invalid Win border color are same --";
       
    else:
      tdkTestObj.setResultStatus("FAILURE");
      print "FAILURE:Win Border color invalid attribute is not set with %s" %tdkTestObj.getResultDetails();
