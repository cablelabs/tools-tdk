...
Test Case ID: CT_RMF_QAMSrc_MPSink_02.</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_RMF_QAMSrc_MPSink_02</test_case_id>
    <test_objective>RMFQAMSrc – To open and close RMFQAMSrc instance when factory method flag is set to false.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XG1</test_setup>
    <pre_requisite>monitorRMF.sh
RmfStreamer
Process should be stopped before runing the script.</pre_requisite>
    <api_or_interface_used>rmf_Error init RMFResult RMFQAMSrc::init_platform()                  RMFQAMSrc::disableCaching();             RMFResult init()     RMFResult open()          RMFResult play()   RMFResult close()     RMFResult term()  RMFResult RMFQAMSrc::uninit_platform()              rmf_Error uninit()</api_or_interface_used>
    <input_parameters>rmf_platform Init(): int- argc, char * argv[]            RMFQAMSrc::init_platform() : None                       RMFQAMSrc::disableCaching() : None                           rmfqamsrc Init(): None           open(): char* uri, char* mimetype play(): float speed, double time.   Close(): None              rmfqamsrc Term(): None   RMFQAMSrc::uninit_platform() : None                                rmf_platform Uninit(): None</input_parameters>
...
11.For each API called in the script, mediaframework agent  will send SUCCESS or FAILURE status to TM via the test agent by comparing the return value of APIs.</automation_approch>
    ...
    <test_script>RMF_QAMSource_OpenClose_02</test_script>
    <skipped>No</skipped>
