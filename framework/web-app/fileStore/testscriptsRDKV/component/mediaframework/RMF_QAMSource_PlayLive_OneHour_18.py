...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1576</id>
  <version>1</version>
  <name>RMF_QAMSource_PlayLive_OneHour_18</name>
  <primitive_test_id>494</primitive_test_id>
  <primitive_test_name>RMF_Element_Create_Instance</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Objective: RMF_QAMSrc – To Play the live content for 1 hour duration. When factory method flag is set to true.
Test CaseId: CT_RMF_QAMSrc_MPSink_18
Test Case Type: Positive.</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_RMF_QAMSrc_MPSink_18</test_case_id>
    <test_objective>RMF_QAMSrc – To Play the live content for 1 hour duration. When factory method flag is set to true.</test_objective>
...
5.On success, Mediaframework agent will call RMFQAMSrc play() for duration 1 hour. 
...
10.For each API called in the script, mediaframework agent  will send SUCCESS or FAILURE status to TM via the test agent by comparing the return value of APIs.</automation_approch>
    ...
    <test_script>RMF_QAMSource_PlayLive_OneHour_18</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RMF_QAMSource_PlayLive_OneHour_18');
                                        ...
                                        if expected_Result in result.upper():
                                                ...
                                                if expected_Result in result.upper():
                                                        ...
                                                        if expected_Result in result.upper():
                                                                ...
                                                                if expected_Result in result.upper():
                                                                        #Make it to play for 1 hour = 60 * 60 secs
                                                                        time.sleep(60*60);
                                                                        src_parameter=["rmfElement"]
                                                                        src_element=["QAMSrc"]
