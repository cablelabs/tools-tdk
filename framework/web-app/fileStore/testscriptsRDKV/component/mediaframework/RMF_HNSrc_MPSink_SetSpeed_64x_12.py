...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>896</id>
  <version>1</version>
  <name>RMF_HNSrc_MPSink_SetSpeed_64x_12</name>
  <primitive_test_id>495</primitive_test_id>
  <primitive_test_name>RMF_Element_Init</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>ALLOCATED</status>
  <synopsis>These Script tests the RDK Mediaframework to REW the live video with 64x in HNSrc MPSink pipeline. Test Case ID: CT_RMF_HNSrc_MPSink_12.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_RMF_HNSrc_MPSink_12</test_case_id>
    <test_objective>RMF_HNSRC_MPSink –Rewind the tsb with 64x  on the HNSrc -MPSink pipeline</test_objective>
...
6.Mediaframework agent will set speed with -64x.
...
9.For each API called in the script, mediaframework agent  will send SUCCESS or FAILURE status to Test Agent by comparing the return vale of APIs.</automation_approch>
    ...
    <test_script>RMF_HNSrc_MPSink_SetSpeed_64x_12</test_script>
    <skipped>No</skipped>
                        ...
                        if Expected_Result in result.upper():
                                ...
                                if Expected_Result in result.upper():
										...
										initialmediatime=float(initialmediatime);
                                                                                #Rewind with 64x
										play_parameter_value=["HNSrc",1,initialmediatime,-64.0]
