...
Test Case ID: CT_RMF_QAMSrc_MPSink_05.</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_RMF_QAMSrc_MPSink_05</test_case_id>
    <test_objective>RMF_QAMSrc – To Get TS Id from PAT when factory method flag is set to true.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XG1</test_setup>
    <pre_requisite>monitorRMF.sh
RmfStreamer
Process should be stopped before runing the script.</pre_requisite>
    <api_or_interface_used>rmf_Error init RMFResult RMFQAMSrc::init_platform()                  RMFQAMSrc::disableCaching();                       RMFQAMSrc* RMFQAMSrc::getQAMSourceInstance()           RMFResult getTSID()     void RMFQAMSrc::freeQAMSourceInstance()           RMFResult RMFQAMSrc::uninit_platform()              rmf_Error uninit()</api_or_interface_used>
    <input_parameters>rmf_platform Init(): int- argc, char * argv[]            RMFQAMSrc::init_platform() : None                       RMFQAMSrc::disableCaching() : None                                         RMFQAMSrc::getQAMSourceInstance(): char* uri                         RMFResult getTSID: unint32_t tsID   RMFQAMSrc::freeQAMSourceInstance(): RMFQAMSrc* uri RMFQAMSrc::uninit_platform() : None                                rmf_platform Uninit(): None</input_parameters>
...
6.On success, Mediaframework agent will call getTSID()() to get TS Id from PAT.
...
10.For each API called in the script, mediaframework agent  will send SUCCESS or FAILURE status to TM via the test agent by comparing the return value of APIs.</automation_approch>
    ...
    <test_script>RMF_QAMSource_GetTsId_05</test_script>
    <skipped>No</skipped>
