...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1588</id>
  <version>6</version>
  <name>RMF_QAMSource_ChangeChannel_SevenTimes_17</name>
  <primitive_test_id>494</primitive_test_id>
  <primitive_test_name>RMF_Element_Create_Instance</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Objective: Tune to a  channel, and change the channel after play back for sometime, and change channel for 4 more times. So, all 5 tuners are cached.  Again change the channel for 7 times. 
Test Case Id: CT_RMF_QAMSrc_MPSink_17
Test Case Type: Positive</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_RMF_QAMSrc_MPSink_17</test_case_id>
    <test_objective>RMFQAMSrc –  Tune to a  channel, and change the channel after play back for sometime, and change channel for 4 more times. So, all 5 tunners are cached. 
Again change the channel for 7 times. So, that all the cacahed channels are reused again discarding the old channels. 
When factory method flag is set to true.</test_objective>
...
10.Repeat the steps fom 6, 7 (with new ocapid's) and 8 for 4 times. 
11.Again repeat the steps fom 6, 7 (with new ocapid's) and 8 for 7 times. 
12.On success, Mediaframework agent will call RMFQAMSrc freeQAMSourceInstance().
13.On success, Mediaframework agent will call RMFQAMSrc uninit_platform().
14.On success, Mediaframework agent will call rmfplatform uninit().
15.For each API called in the script, mediaframework agent  will send SUCCESS or FAILURE status to TM via the test agent by comparing the return value of APIs.</automation_approch>
...
Checkpoint 2. Search the log of the API() calls to see “SPTS Read Timeout Error” as occured or not.</except_output>
    ...
    <test_script>RMF_QAMSource_ChangeChannel_SevenTimes_17</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'RMF_QAMSource_ChangeChannel_SevenTimes_17');
...
def ChannelChange(index, numOfTimesChannelChange,result):
                                        ...
                                        if expected_Result in result.upper():
                                                ...
                                                if expected_Result in result.upper():
                                                        ...
                                                        if expected_Result in result.upper():
                                                                ...
                                                                print "RMF_Element_Pause failed"
                                                                return result
                                                else:
                                                        print "RMF_Element_Play failed"
                                                        return result
                                        else:
                                                print "RMF_Element_Sink_SetSource failed"
                                                return result
                                else:
                                        print "RmfElement_QAMSrc_ChangeURI failed"
                                        return result
                        else:
                                print "RMF_Element_Create_Instance failed"
                                return result
                else:
...
def ReleaseQamInstance(index,numOfTimesChannelChange):
        global expected_Result
        global obj
        while index < numOfTimesChannelChange:
                src_parameter=["rmfElement","factoryEnable","newQamSrc","numOfTimeChannelChange"]
                                                        ...
                                                        if expected_Result in result.upper():
                                                                ...
                                                                if expected_Result in result.upper():
                                                                        ...
                                                                        if expected_Result in result.upper():
                                                                                ...
                                                                                #Channel Change for 4 times
                                                                                result = ChannelChange(index,numOfTimesChannelChange,result)
                                                                                if expected_Result in result.upper():
                                                                                        index = 0
                                                                                        #Release Qam insatnce allocated
                                                                                        ReleaseQamInstance(index,numOfTimesChannelChange)

                                                                                        print "Changing the channel for 7 more times"

                                                                                        #Repeat the channel chang fo 7 times
                                                                                        index = 0
                                                                                        numOfTimesChannelChange = 7
                                                                                        result = ChannelChange(index,numOfTimesChannelChange,result)
                                                                                        if expected_Result in result.upper():
                                                                                                index = 0
                                                                                                #Release Qam insatnce allocated
