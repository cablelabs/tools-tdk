...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>884</id>
  <version>2</version>
  <name>RMF_HNSrc_MPSink_FF_4x_21</name>
  <primitive_test_id>495</primitive_test_id>
  <primitive_test_name>RMF_Element_Init</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>These Script tests the RDK Mediaframework video trickplay for 4x on live tsb. Test Case ID: CT_RMF_HNSrc_MPSink_21.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_RMF_HNSRC_MPSink_21</test_case_id>
    <test_objective>RMF_HNSRC_MPSink –Fast forward the tsb with 4x  on the HNSrc -MPSink pipeline</test_objective>
...
6.Mediaframework agent will set speed with 4x.
...
9.For each API called in the script, mediaframework agent  will send SUCCESS or FAILURE status to Test Agent by comparing the return vale of APIs.</automation_approch>
    ...
    <test_script>RMF_HNSrc_MPSink_FF_4x_21</test_script>
    <skipped>No</skipped>
                        ...
                        if Expected_Result in result.upper():
								...
								time.sleep(30)
										...
										if Expected_Result in result.upper():
											...
											if Expected_Result in result.upper():
												#FF with 4x
												play_parameter_value=["HNSrc",1,0,4.0] 
												result=Create_and_ExecuteTestStep('RMF_Element_Play',obj,Expected_Result,play_parameter_name,play_parameter_value);
												if Expected_Result in result.upper():
