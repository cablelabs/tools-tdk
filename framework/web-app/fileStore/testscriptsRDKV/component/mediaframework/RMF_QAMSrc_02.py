...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1135</id>
  <version>1</version>
  <name>RMF_QAMSrc_02</name>
  <primitive_test_id>494</primitive_test_id>
  <primitive_test_name>RMF_Element_Create_Instance</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>RMFQAMSrc – To call open() api by passing valid dvrlocator as input to its parameter and close RMFQAMSrc instance when factory method flag is set to false.
Test caseID: CT_RMF_QAMSrc_02</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_RMF_QAMSrc_02</test_case_id>
    <test_objective>RMFQAMSrc – To call open() api by passing valid dvrlocator as input to its parameter and close RMFQAMSrc instance when factory method flag is set to false.</test_objective>
    <test_type>Negative</test_type>
    <test_setup>XG1</test_setup>
    <pre_requisite>monitorRMF.sh
RmfStreamer
Process should be stopped before runing the script.</pre_requisite>
    <api_or_interface_used>rmf_Error init RMFResult RMFQAMSrc::init_platform()      RMFResult init()     
RMFResult open()         RMFResult close()     RMFResult term()  RMFResult RMFQAMSrc::uninit_platform()  rmf_Error uninit()</api_or_interface_used>
    <input_parameters>rmf_platform Init(): int- argc, char * argv[]            RMFQAMSrc::init_platform() : None                       
rmfqamsrc Init(): None           open(): char* uri, char* mimetype close(): None              rmfqamsrc Term(): None   RMFQAMSrc::uninit_platform() : None                                rmf_platform Uninit(): None</input_parameters>
...
6.On success, Mediaframework agent will call open() of  QAMSrc element by passing valid dvrlocator url value as input to its parameter.
7.On success, Mediaframework agent will call close() of  QAMSrc element.
8.On success, Mediaframework agent will call term() and de-initialize the QAMSrc element.
...
11.For each API called in the script, mediaframework agent  will send SUCCESS or FAILURE status to TM via the test agent by comparing the return value of APIs.</automation_approch>
    ...
    <test_script>RMF_QAMSrc_02</test_script>
    <skipped>No</skipped>
...
def Create_and_ExecuteTestStep(teststep, testobject, expectedresult,parametername, parametervalue):
    ...
    if expectedresult.upper() == result.upper():
                ...
                if "SUCCESS" in result.upper():
                        ...
                        if "SUCCESS" in result.upper():
                                ...
                                if "SUCCESS" in result.upper():
                                        ...
                                        result=Create_and_ExecuteTestStep('RMF_Element_Open',obj,"FAILURE",src_parameter,src_element);
                                        if "SUCCESS" in result.upper():
                                                src_parameter=["rmfElement"]
                                                src_element=["QAMSrc"]
