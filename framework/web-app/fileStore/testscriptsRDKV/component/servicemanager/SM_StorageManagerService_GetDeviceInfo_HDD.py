...
<xml>
  ...
  <name>SM_StorageManagerService_GetDeviceInfo_HDD</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the result of service manager API getDeviceInfo contains information about HDD</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Checks if the result of service manager API getDeviceInfo contains information about HDD</test_objective>
...
4. TM will check if the API call result contains information about HDD and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the API getDeviceInfo contains information about HDD</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetDeviceInfo_HDD</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetDeviceInfo_HDD');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetDeviceInfo_HDD');
				...
				devInfoList = [];
					...
					for key in devInfoList:
						if devInfoList[key].has_key('deviceType'):
							if devInfoList[key]['deviceType'] == 0:
								status = True;
								break;
					if status:
						tdkTestObj.setResultStatus("SUCCESS");
						print "Device Info contains HDD details";
					else:
						tdkTestObj.setResultStatus("FAILURE");
						print "Device Info does not contain HDD details";
				else:
					tdkTestObj.setResultStatus("FAILURE");
