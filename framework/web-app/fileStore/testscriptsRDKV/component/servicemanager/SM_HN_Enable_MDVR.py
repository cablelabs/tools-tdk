...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if MDVR status can be enabled using Service manager API</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_55</test_case_id>
    <test_objective>Checks if MDVR status can be enabled using Service manager API</test_objective>
...
Checkpoint 2. Check the value retrieved using isMdvrEnabled API is same as the value set using setMdvrEnabled API.</except_output>
    ...
    <test_script>SM_HN_Enable_MDVR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_HN_Enable_MDVR');
