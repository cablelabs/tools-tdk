...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>SM_System_Persist_SetandGet_PowerState_On</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>7</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether power state is ON is persisting or not</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_81</test_case_id>
    <test_objective>To check whether set and get the ON power state using system service persisting or not</test_objective>
...
5.set the power state as ON. Then un register and register the  service.
...
Checkpoint 2.Check the power state is set as ON</except_output>
    ...
    <test_script>SM_System_Persist_SetandGet_PowerState_On</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_Persist_SetandGet_PowerState_On');
iarmObj.configureTestCase(ip,port,'SM_System_Persist_SetandGet_PowerState_On');
                    ...
                    if "SUCCESS" in register:
                        ...
                        if expectedresult in actualresult:
                            ...
                            if methodDetail == powerState:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "Power state " ,powerState , "is persisted "
                            else:
                                print "Power state" , powerState , "is not persisted"
                                tdkTestObj.setResultStatus("FAILURE");
                        else:
