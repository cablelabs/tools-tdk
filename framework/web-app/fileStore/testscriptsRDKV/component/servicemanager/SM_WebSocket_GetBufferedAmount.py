...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1529</id>
  <version>1</version>
  <name>SM_WebSocket_GetBufferedAmount</name>
  <primitive_test_id>650</primitive_test_id>
  <primitive_test_name>SM_WebSocket_GetBufferedAmount</primitive_test_name>
  <primitive_test_version>0</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify get buffered amount on WebSocketService.
Test Case ID: CT_Service Manager_29</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_29</test_case_id>
    <test_objective>To verify get buffered amount on WebSocketService</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XG1-1</test_setup>
    <pre_requisite>None</pre_requisite>
    <api_or_interface_used>ServiceManager::getGlobalService
WebSocketService::bufferedAmount</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1. TM loads the Service_Manager_Agent via the test agent.
2.Service_Manager_Agent will register DeviceSettingService with ServiceManager component.
3.On Success of registerService , Service_Manager_Agent will get buffered amount info from WebSocketService bufferedAmount method.
5.Service_Manager_Agent will deregister a given service from ServiceManager component.
6. Service_Manager_Agent will check get buggered amount status and return SUCCESS/FAILURE status.</automation_approch>
...
2.TestMgr_SM_WebSocket_GetBufferedAmount
3.TestMgr_SM_UnRegisterService</test_stub_interface>
    <test_script>SM_WebSocket_GetBufferedAmount</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'CT_Service Manager_29');
...
if "SUCCESS" in loadmodulestatus.upper():
        ...
        if expectedresult in actualresult:
                ...
                #Call GetBufferedAmount API
                tdkTestObj = obj.createTestStep('SM_WebSocket_GetBufferedAmount');
                expectedresult="SUCCESS"
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                serviceDetail = tdkTestObj.getResultDetails();
                print "[TEST EXECUTION DETAILS] : %s"%serviceDetail;
                #Check for SUCCESS/FAILURE return value of SM_WebSocket_GetBufferedAmount
                if expectedresult in actualresult:
                        print "SUCCESS: GetBufferedAmount successful";
                        tdkTestObj.setResultStatus("SUCCESS");
                else:
                        print "FAILURE: GetBufferedAmount failure";
