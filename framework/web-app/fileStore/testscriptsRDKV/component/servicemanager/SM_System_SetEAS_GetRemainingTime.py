...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>SM_System_SetEAS_GetRemainingTime</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set the EAS mode and get the remaining time for which it stays in that mode</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_118</test_case_id>
    <test_objective>To set EAS mode with positive duration and to get the remaining time for which it stays in EAS mode using system service</test_objective>
...
5.Service_Manager_Agent will invoke the api getMode after sometime and verify that the duration is less than the input duration give by the user 
...
Checkpoint 2.Check the  mode set with setMode by getting it using getMode</except_output>
    ...
    <test_script>SM_System_SetEAS_GetRemainingTime</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_SetEAS_GetRemainingTime');
iarmObj.configureTestCase(ip,port,'SM_System_SetEAS_GetRemainingTime');
        ...
        if "SUCCESS" in connect:
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    paramValue = {"duration":50,"mode":"EAS"}
                    original_dur = paramValue.get("duration");
                    methodName="setMode"
                    tdkTestObj.addParameter("service_name", serviceName);
			...
			time.sleep(2);
                        ...
                        if expectedresult in actualresult:
                            ...
                            getDuration = getModeValue.split(",");
			    remaining_dur = getDuration[0].split(":")[1];			    
                            if remaining_dur < original_dur:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "Remaining time before the mode switches back to NORMAL mode is: ",remaining_dur

                            else:
                                print methodName, "FAILURE: Remaining duration should be less than original duration";
