...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>SM_HN_Persist_Disable_VPOP</name>
  <primitive_test_id/>
  <primitive_test_name>SM_HN_IsVPOPEnabled</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether VPOP disable set using home networking service persist even after unregistering the service</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_68</test_case_id>
    <test_objective>To check whether VPOP disable set using home networking service persist even after unregistering the service</test_objective>
...
Checkpoint 2. Check the value retrieved using is_vpop_enabled API is same as the value set using set_vpop_enabled API.</except_output>
    ...
    <test_script>SM_HN_Persist_Disable_VPOP</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_HN_Persist_Disable_VPOP');
...
if "SUCCESS" in smLoadStatus.upper():
        ...
        if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    tdkTestObj.executeTestCase(expectedresult);
                    actualresult = tdkTestObj.getResult();
                    disableVPOPDetails = tdkTestObj.getResultDetails();
                    print "[TEST EXECUTION DETAILS 1] : ",disableVPOPDetails;
                    if expectedresult in actualresult:
                        #Check whether VPOP is enabled or not
                        if str(enableValue) in disableVPOPDetails:
                            tdkTestObj.setResultStatus("SUCCESS");
                    ...
                    print "Unregistering the homeNetworkingService to check VPOP disable persists after registering the service again"
                    unregister = servicemanager.unRegisterService(smObj,serviceName);
                 
                    #Register the home networking service
                    print "Registering the homeNetworkingService and checking whether VPOP disable persists or not"
                    register = servicemanager.registerService(smObj,serviceName)
                    if "SUCCESS" in register:
                        #Get the VPOP status
