...
<xml>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_58</test_case_id>
    <test_objective>Checks if Upnp status can be disabled using Service manager API</test_objective>
...
3. On Success of registerService , Service_Manager_Agent will invoke "setUpnpEnabled" API to set the value of Upnp status to "false".
...
Checkpoint 2. Check the value retrieved using isUpnpEnabled API is same as the value set using setUpnpEnabled API.</except_output>
    ...
    <test_script>SM_HN_Disable_Upnp</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_HN_Disable_Upnp');
