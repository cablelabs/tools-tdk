...
<xml>
  ...
  <name>SM_DisplaySettings_GetSupportedVideoDisplays</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getSupportedVideoDisplays returns a list of supported display devices</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_126</test_case_id>
    <test_objective>Checks if the service manager API getSupportedVideoDisplays returns a list of supported display devices</test_objective>
...
CallMethod : const QString - "getSupportedVideoDisplays" ,const ServiceParams - bool
...
4. TM will check if the API returned a non empty list and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the API returned a non empty list.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_DisplaySettings_GetSupportedVideoDisplays</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_DisplaySettings_GetSupportedVideoDisplays');
dsObj.configureTestCase(ip,port,'SM_DisplaySettings_GetSupportedVideoDisplays');
	...
	result = devicesettings.dsManagerInitialize(dsObj);
        if "SUCCESS" in result:
			...
			if expectedresult in actualresult:
				...
				if supportedDisplays != []:
					tdkTestObj.setResultStatus("SUCCESS");
		                        print "Supported Video Displays retrieved successfully \n";
				else:
					tdkTestObj.setResultStatus("FAILURE");
                                        print "Empty value retrieved for Supported Video Displays\n";
			else:	
				tdkTestObj.setResultStatus("FAILURE");
