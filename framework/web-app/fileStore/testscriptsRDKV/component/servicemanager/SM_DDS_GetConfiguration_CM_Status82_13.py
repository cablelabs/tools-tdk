...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_DDS_GetConfiguration_CM_Status82_13');
snmpObj.configureTestCase(ip,port,'SM_DDS_GetConfiguration_CM_Status82_13');
...
if "SUCCESS" in smLoadStatus.upper() and "SUCCESS" in snmpLoadStatus.upper():
		...
		ecmIp = getECMIp();
                if ecmIp != "":
                        ...
                        if commString != "":
                                actResponse =snmplib.SnmpExecuteCmd("snmpwalk", commString, "-v 2c", "1.3.6.1.4.1.4491.2.1.20.1.2.1.6", ecmIp);
                                if "4491.2.1.20.1.2.1.6.82" in actResponse:
                                        #Prmitive test case which associated to this Script
                                        tdkTestObj = smObj.createTestStep('SM_DDS_GetConfiguration');
                                        names = "Device.X_RDKCENTRAL-COM_DocsIf.docsIf3CmStatus_82";
