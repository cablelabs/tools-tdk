...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>SM_System_SetandGet_EAS_RebootPersistent</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set the EAS mode, reboot the box and then get the mode</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_122</test_case_id>
    <test_objective>To set EAS mode , reboot the box and then get the mode using system service </test_objective>
...
4.Service_Manager_Agent will invoke the api setMode with mode as EAS and duration as a negative value
5.Reboot the box
6.Service_Manager_Agent will invoke the api getMode 
7.Depending upon the return value of getMode api, SUCCESS/FAILURE status is returned
...
Checkpoint 2.Check the  mode set with setMode by getting it using getMode</except_output>
    ...
    <test_script>SM_System_SetandGet_EAS_RebootPersistent</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_SetandGet_EAS_RebootPersistent');
iarmObj.configureTestCase(ip,port,'SM_System_SetandGet_EAS_RebootPersistent');
        ...
        if "SUCCESS" in connect:
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    paramValue = {"duration":1000,"mode":"EAS"}
                    methodName="setMode"
                    tdkTestObj.addParameter("service_name", serviceName);
			                        ...
			                        print "Set the API version %s succesfully" % apiVersion
                                                ...
                                                if expectedresult in actualresult:
                                                    ...
                                                    if "NORMAL" in getModeValue:
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print "Successfully switched to the NORMAL mode after reboot"
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print "Unable to switch to the NORMAL mode after reboot"
            ...
            #Unloading service manager module^M
