...
<xml>
  ...
  <name>SM_FP_setBlink_valid_2424</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>SM_FP_SetBlink</primitive_test_name>
  <!--  -->
  <primitive_test_version>2</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>SM_FP_setBlink_valid_
RDK-2424</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'SM_FP_setBlink_valid_2424');
dsobj.configureTestCase(ip,port,'SM_FP_setBlink_valid_2424');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
        ...
        if "SUCCESS" in result.upper():
                    ...
                    if expectedresult in actualresult:
                            ...
                            tdkTestObj = obj.createTestStep('SM_FP_SetBlink');
                            expectedresult="SUCCESS"
                            sequencecount = 2;
                            RedColor='';
                            BlueColor='';
                            ...
                            for x in range(0, sequencecount):
                                    ...
                                    RedColor = RedColor+str(Red)+',';
                                    BlueColor = BlueColor+str(Blue)+',';
                            ...
                            tdkTestObj.addParameter("BlinkDuration",blinkduration);
                            print "RedColor = %s, BlueColor = %s , GreenColor =%s , setbrightness = %s ,  blink=%s"%(RedColor,BlueColor,GreenColor,setbrightness,blinkduration);
                            tdkTestObj.executeTestCase(expectedresult);
                            actualresult= tdkTestObj.getResult();
                            brightnessdetails= tdkTestObj.getResultDetails();
                            #Check for SUCCESS/FAILURE return value of SM_FP_SetBlink
                            if expectedresult in actualresult:
                                    print "SUCCESS: Application succesfully executes SM_FP_SetBlink API";
                                    ...
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print "FAILURE: Application Failed to execute SM_FP_SetBlink API";
