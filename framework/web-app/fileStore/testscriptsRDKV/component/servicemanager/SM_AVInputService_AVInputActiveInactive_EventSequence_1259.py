...
<xml>
  ...
  <name>SM_AVInputService_AVInputActiveInactive_EventSequence_1259</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Invokes AVInputActiveEvent and AVInputInactiveEvent event sequentially
  TestCase ID : CT_1259_5</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_AVInputService_AVInputActiveInactive_EventSequence_1259');
...
if "SUCCESS" in smLoadStatus.upper():
	...
	ret = servicemanager.RunSMEvent(smObj, service_name, event_name, event_param);
		...
		if expectedresult in ret:
			...
			print "FAILURE: AVInputInactive event not recieved"
	else:
