...
<xml>
  ...
  <name>SM_System_SetandGet_EAS_ZeroDuration</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set WAREHOUSE mode with duration -1, then set the mode to EAS with duration			 0 and verify if the current mode is changed to EAS or not.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_114</test_case_id>
    <test_objective>To set EAS mode with zero duration and to verify that the mode is not changed using system service</test_objective>
...
4.Service_Manager_Agent will invoke the api setMode with mode as EAS and duration as zero 
5.Service_Manager_Agent will invoke the api getMode and verify that the mode is not changed
...
Checkpoint 2.Check the  mode set with setMode by getting it using getMode</except_output>
    ...
    <test_script>SM_System_SetandGet_EAS_ZeroDuration</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_SetandGet_EAS_ZeroDuration');
iarmObj.configureTestCase(ip,port,'SM_System_SetandGet_EAS_ZeroDuration');
        ...
        if "SUCCESS" in connect:
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "SUCCESS:", methodName, " call is successful with" ,paramValue;
			...
			#Get the currently set mode
                        ...
                        if expectedresult in actualresult:
                            ...
                            if "WAREHOUSE" in getModeValue:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "Successfully sets the WAREHOUSE mode"

   		               #Set the EAS mode with duration zero
                                tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
                                paramValue = {"duration":0,"mode":"EAS"}
                                methodName="setMode"
                                tdkTestObj.addParameter("service_name", serviceName);
                                tdkTestObj.addParameter("method_name", methodName);
                                tdkTestObj.addParameter("params", paramValue);
				    ...
				    #Get the previously set mode
                        	    ...
                        	    if expectedresult in actualresult:
                	                ...
                	                if "WAREHOUSE" in getModeValue:
                        	            tdkTestObj.setResultStatus("SUCCESS");
                                	    print "Successfully retains the initial WAREHOUSE mode"
	                                else:
        	                            tdkTestObj.setResultStatus("FAILURE");
                	                    print "Unable to retain the initial WAREHOUSE mode"
                        	    else:
                                    ...
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print "setMode call is NOT successful with ",paramValue;

                            else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print "Unable to get the WAREHOUSE mode"
