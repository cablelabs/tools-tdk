...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>SM_System_SetandGet_EAS_MultipleDurations</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set EAS mode with different durations so as to check if second duration would override or add up with the previous duration</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_120</test_case_id>
    <test_objective>To set EAS mode multiple times with different durations so as to check if the durationa are added or overwritten.</test_objective>
...
5.Again invoke the setMode api to set EAS mode with a different duration
5.Service_Manager_Agent will invoke the api getMode twice, one after duration1 time and another after duration 2 time and verify that the mode is switched to NORMAL 
...
Checkpoint 2.Check the  mode set with setMode by getting it using getMode</except_output>
    ...
    <test_script>SM_System_SetandGet_EAS_MultipleDurations</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_SetandGet_EAS_MultipleDurations');
iarmObj.configureTestCase(ip,port,'SM_System_SetandGet_EAS_MultipleDurations');
        ...
        if "SUCCESS" in connect:
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    paramValue = {"duration":80,"mode":"EAS"}
                    methodName="setMode"
                    tdkTestObj.addParameter("service_name", serviceName);
                    ...
                    if expectedresult in actualresult:
                        ...
                        if expectedresult in actualresult:
                            ...
                            if "EAS" in getModeValue:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "Successfully set to EAS mode"

				#Trying to set EAS with a different duration
                                tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
                                paramValue = {"duration":60,"mode":"EAS"}
                                methodName="setMode"
                                tdkTestObj.addParameter("service_name", serviceName);
                                tdkTestObj.addParameter("method_name", methodName);
                                tdkTestObj.addParameter("params", paramValue);
				    ...
				    #Get the mode after duration seconds to see if it is reverted to NORMAL mode
                                    tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
                                    expectedresult="SUCCESS"
                                    methodName="getMode"
                                    tdkTestObj.addParameter("service_name", serviceName);
                                    ...
                                    if expectedresult in actualresult:
                                        ...
                                        if "NORMAL" in getModeValue:
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print "Successfully switched to the NORMAL mode after lesser input duration"
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
                                            print "Unable to switch to the NORMAL mode after lesser input duration"

  					    #Sleep for remaining duration (80-60=20)
                                            time.sleep(20);
                                            ...
                                            if expectedresult in actualresult:
                                                ...
                                                if "NORMAL" in getModeValue:
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    print "Successfully switched to the NORMAL mode after greater input duration"
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
                                                    print "Unable to switch to the NORMAL mode after greater input duration"

         					    #Sleep for the remaining time of the sum of the durations (140-80=60)
                                                    time.sleep(60);
                        	                    ...
                        	                    if expectedresult in actualresult:
                                                	...
                                                	if "NORMAL" in getModeValue:
	                                                    tdkTestObj.setResultStatus("SUCCESS");
        	                                            print "Successfully switched to the NORMAL mode after sum of the input durations"
                	                                else:
                        	                            tdkTestObj.setResultStatus("FAILURE");
                                	                    print "Unable to switch to the NORMAL mode after sum of the input durations"
				    ...
				    print methodName, "call is NOT successful with ",paramValue;
                                ...
                                print methodName, "could not set EAS mode";
                                tdkTestObj.setResultStatus("FAILURE");
