...
<xml>
  ...
  <name>SM_DisplaySettings_GetSupportedResolutionsCheck</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getSupportedResolutions returns a non empty list of supported resolutions</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_128</test_case_id>
    <test_objective>Checks if the service manager API getSupportedResolutions returns a non empty list of supported resolutions</test_objective>
...
CallMethod : const QString - "getSupportedResolutions" ,const ServiceParams - bool
...
3. On Success of registerService , Service_Manager_Agent will invoke "getSupportedResolutions" API to get the list of supported resolutions.
4. TM will check if the API call is SUCCESS and it returned a non empty list and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the API getSupportedResolutions returns a non empty list of supported resolutions.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_DisplaySettings_GetSupportedResolutionsCheck</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_DisplaySettings_GetSupportedResolutionsCheck');
dsObj.configureTestCase(ip,port,'SM_DisplaySettings_GetSupportedResolutionsCheck');
	...
	result = devicesettings.dsManagerInitialize(dsObj);
        if "SUCCESS" in result:
			...
			tdkTestObj = smObj.createTestStep('SM_SetAPIVersion');
		        ...
		        expectedresult="SUCCESS"
                	tdkTestObj.addParameter("service_name", serviceName);
			tdkTestObj.addParameter("method_name", "getSupportedResolutions");
                	tdkTestObj.executeTestCase(expectedresult);
			...
			if expectedresult in actualresult:
				tdkTestObj.setResultStatus("SUCCESS");
			        resultDetails = tdkTestObj.getResultDetails();
                		supportedResolutions = json.loads(resultDetails);
		        	print "Supported resolutions: ", supportedResolutions;
				if supportedResolutions !=[]:
					tdkTestObj.setResultStatus("SUCCESS");
					print "Retrieved non empty list of supported resolutions \n";
				else:
					tdkTestObj.setResultStatus("FAILURE");
					print "Retrieved an empty list for supported resolutions \n";
			else:	
				tdkTestObj.setResultStatus("FAILURE");
                		print "Failed to get supported resolutions \n";
