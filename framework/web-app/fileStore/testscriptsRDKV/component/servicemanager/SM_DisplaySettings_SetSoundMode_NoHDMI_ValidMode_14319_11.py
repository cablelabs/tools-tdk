...
<xml>
  ...
  <name>SM_DisplaySettings_SetSoundMode_NoHDMI_ValidMode_14319_11</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test setSoundMode on unconnected HDMI port with a valid audio mode
  Test Case Id : CT_14319_11</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
dsObj.configureTestCase(ip,port,'SM_DisplaySettings_SetSoundMode_NoHDMI_ValidMode_14319_11');
...
obj.configureTestCase(ip,port,'SM_DisplaySettings_SetSoundMode_NoHDMI_ValidMode_14319_11');
...
if "SUCCESS" in result.upper():
		...
		if expectedresult in get_ret[0]:
			...
			if get_ret[1]=="passthru":
                ...
                #Check for SUCCESS/FAILURE return value of SM_DisplaySetting_GetSoundMode
                if expectedresult in actualresult:
                        print "SUCCESS: GetSoundMode() successful";
			if serviceDetail!=audioMode :
				print "SUCCESS: audiomode not set for disconnected port"
	                        tdkTestObj.setResultStatus("SUCCESS");
			else:
                                print "FAILURE: audiomode set for disconnected port"
                                tdkTestObj.setResultStatus("FAILURE");				
                else:
