...
<xml>
  ...
  <name>SM_StorageManagerService_GetTSBMaxMinutes</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getTSBMaxMinutes returns value within border range</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_105</test_case_id>
    <test_objective>Checks if the service manager API getTSBMaxMinutes returns value within border range</test_objective>
...
CallMethod : const QString - "getTSBMaxMinutes/getTSBCapacityMinutes" ,const ServiceParams - bool
...
6. Service_Manager_Agent will check if TSBMaxMinutes value is within the range 0 and TSBCapacityMinutes and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the return value TSBMaxMinutes is within the range 0 and TSBCapacityMinutes.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetTSBMaxMinutes</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetTSBMaxMinutes');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetTSBMaxMinutes');
		...
		#Calling IARM Bus Init
                ...
                if "SUCCESS" in init:
					...
					tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
						...
						if tsbMaxMinutes >= 0 and tsbMaxMinutes <= int(tsbCapacityMinutes):
							print "TSBMaxMinutes value within range";
							tdkTestObj.setResultStatus("SUCCESS");
						else:
							print "Wrong TSBMaxMinutes value beyond range";
					...
					tdkTestObj.setResultStatus("FAILURE");
                        		print "Failed to get TSBCapaciyMinutes \n";
