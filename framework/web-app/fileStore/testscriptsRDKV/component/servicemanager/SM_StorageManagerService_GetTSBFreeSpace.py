...
<xml>
  ...
  <name>SM_StorageManagerService_GetTSBFreeSpace</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getTSBFreeSpace returns valid value</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_101</test_case_id>
    <test_objective>Checks if the service manager API getTSBFreeSpace returns valid value</test_objective>
...
CallMethod : const QString - "getTSBFreeSpace/getTSBCapacity" ,const ServiceParams - bool
...
3. On Success of registerService , Service_Manager_Agent will invoke "getTSBFreeSpace" API to get the value of TSBFreeSpace.
4. Service_Manager_Agent will invoke "getTSBCapacity" API to get the value of TSBCapacity.
5. Service_Manager_Agent will check if the value of TSBFreeSpace is less than or equal to the value of TSBCapacity and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the value of TSBFreeSpace is less than or equal to the value of TSBCapacity.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetTSBFreeSpace</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetTSBFreeSpace');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetTSBFreeSpace');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in iarmLoadStatus.upper():
		...
		#Calling IARM Bus Init
                ...
                if "SUCCESS" in init:
		                ...
		                tdkTestObj.addParameter("method_name", "getTSBFreeSpace");
                		tdkTestObj.executeTestCase(expectedresult);
		                #Get the result of execution
                		actualresult = tdkTestObj.getResult();
		                if expectedresult in actualresult:
		                        resultDetails = tdkTestObj.getResultDetails();
                		        tsbFreeSpace = json.loads(resultDetails);
		                        print "TSBFreeSpace : ", tsbFreeSpace;
					tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
        	                        tdkTestObj.addParameter("service_name", serviceName);
					...
					if expectedresult in actualresult:
						...
						print tsbCapacity;
						if tsbFreeSpace <= tsbCapacity:
							tdkTestObj.setResultStatus("SUCCESS");
							print "TSBCapacity is valid";
						else:
							tdkTestObj.setResultStatus("FAILURE");
							print "TSBCapacity is not valid";
					else:
						tdkTestObj.setResultStatus("FAILURE");
	                                        print "Failed to get TSBCapacity";
				else:
					tdkTestObj.setResultStatus("FAILURE");
                        		print "Failed to get TSBFreeSpace \n";
