...
<xml>
  ...
  <name>SM_StorageManagerService_SetDVREnabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API setDVREnabled sets DVR enable status successfully</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_94</test_case_id>
    <test_objective>Checks if the service manager API setDVREnabled sets DVR enable status successfully</test_objective>
...
CallMethod : const QString - "isDVREnabled/setDVREnabled" ,const ServiceParams - bool
...
3. On Success of registerService , Service_Manager_Agent will invoke "isDVREnabled" API to check if DVR is enabled/disabled.
4. Service_Manager_Agent will check if DVR is enabled or disabled and will invoke "setDVREnabled" API to set DVR status to disable or enable respectively 
5. Service_Manager_Agent will invoke "isDVREnabled" API to check if DVR status is same as the value set in step 4.
6. Repeat step 4 once again to change the DVR status.
7. Service_Manager_Agent will invoke "isDVREnabled" API to check if DVR status is same as the value set in step 6 and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the return value of the get API is same as the one set using set API.</except_output>
    ...
    <test_script>SM_StorageManagerService_SetDVREnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_SetDVREnabled');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_SetDVREnabled');


def changeDVRStatus(valueToEnable, serviceName):
	print "Setting DVR status to %s" %valueToEnable;
	success = False;
        expectedresult="SUCCESS"
        tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
        tdkTestObj.addParameter("service_name", serviceName);
        tdkTestObj.addParameter("method_name", "setDVREnabled");
                ...
                if expectedresult in actualresult:
                	tdkTestObj.setResultStatus("SUCCESS");
                        ...
                        if enableStatus == valueToEnable:
                        	tdkTestObj.setResultStatus("SUCCESS");
                                print "DVR status changed successfully";
				success = True;
                        else:
                        	tdkTestObj.setResultStatus("FAILURE");
                                print "DVR status not changed";
		else:
                	tdkTestObj.setResultStatus("FAILURE");
                        print "Failed to get DVR enable status \n";
	else:
        	tdkTestObj.setResultStatus("FAILURE");
                print "Failed to set DVR enable status \n";
	return success;
...
if "SUCCESS" in smLoadStatus.upper() and "SUCCESS" in iarmLoadStatus.upper():
		...
		#Calling IARM Bus Init
                ...
                if "SUCCESS" in init:
					...
					result = changeDVRStatus(enable,serviceName);
					if result:
						result = changeDVRStatus(enableStatus,serviceName);		
				else:
					tdkTestObj.setResultStatus("FAILURE");
                        		print "Failed to get DVR enable status \n";
