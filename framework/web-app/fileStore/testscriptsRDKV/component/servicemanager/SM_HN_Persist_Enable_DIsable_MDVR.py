...
<xml>
  ...
  <name>SM_HN_Persist_Enable_DIsable_MDVR</name>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_62</test_case_id>
    <test_objective>Checks if MDVR enable/disable status persists across different instances of service.</test_objective>
...
CallMethod : const QString - "setMdvrEnabled"/"isMdvrEnabled" ,const ServiceParams - bool
UnregisterService : Qstring-serviceName</input_parameters>
    <automation_approch>1. TM loads the Service_Manager_Agent via the test agent.
2. Service_Manager_Agent will register "homeNetworkingService" with ServiceManager component.
3. On Success of registerService , Service_Manager_Agent will invoke "setMdvrEnabled" API to set the value of MDVR status to "true".
4. Service_Manager_Agent will invoke "isMdvrEnabled" API to get the value of MDVR status.
...
8. Service_Manager_Agent will invoke "isMdvrEnabled" API to get the value of MDVR status.
9. TM will check if the value is "true" and return SUCCESS/FAILURE status.
10. Service_Manager_Agent will invoke "setMdvrEnabled" API to set the value of MDVR status to "false".
11. Service_Manager_Agent will invoke "isMdvrEnabled" API to get the value of MDVR status.
...
15. Service_Manager_Agent will invoke "isMdvrEnabled" API to get the value of MDVR status.
...
Checkpoint 2. Check the enable/disable values persist across different instances of service</except_output>
    ...
    <test_script>SM_HN_Persist_Enable_DIsable_MDVR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_HN_Persist_Enable_DIsable_MDVR');
...
if "SUCCESS" in smLoadStatus.upper():
	serviceName = "homeNetworkingService";
        ...
        if "SUCCESS" in register:
        	#Enable MDVR status.
                tdkTestObj = smObj.createTestStep('SM_HN_EnableMDVR');
                expectedresult = "SUCCESS";
                enable = 1;
                print "Setting MDVR status to ",enable;
		...
		#Get MDVR enable status
                tdkTestObj = smObj.createTestStep('SM_HN_IsMDVREnabled');
                expectedresult = "SUCCESS";
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                ...
                if expectedresult in actualresult:
                	#Compare the set value with get value
                        if enable == int(getEnabledDetails):
                        	tdkTestObj.setResultStatus("SUCCESS");
                                print "MDVR status enabled";
                        else:
                        	tdkTestObj.setResultStatus("FAILURE");
                                print "MDVR status not enabled"
		else:
        		tdkTestObj.setResultStatus("FAILURE");

		#Unregistering and re registering service to check persistence of MDVR status
		...
		register = servicemanager.registerService(smObj,serviceName)
			...
			#Get MDVR enable status
	                ...
	                if expectedresult in actualresult:
                	        ...
                	        if enable == int(getEnabledDetails):
                        	        tdkTestObj.setResultStatus("SUCCESS");
                                	print "MDVR Enable status Persists";
	                        else:
        	                        tdkTestObj.setResultStatus("FAILURE");
                	                print "MDVR status does not Persist"
	                else:
        	                tdkTestObj.setResultStatus("FAILURE");
			...
			#Get MDVR enable status
	                ...
	                if expectedresult in actualresult:
        	        	#Compare the set value with get value
                	        if enable == int(getEnabledDetails):
                        		tdkTestObj.setResultStatus("SUCCESS");
                                	print "MDVR status disabled";
	                        else:
        	                	tdkTestObj.setResultStatus("FAILURE");
                	                print "MDVR status not disabled";
			...
			register = servicemanager.registerService(smObj,serviceName)
				...
				#Get MDVR enable status
                	        ...
                	        if expectedresult in actualresult:
                                	...
                                	if enable == int(getEnabledDetails):
                                        	tdkTestObj.setResultStatus("SUCCESS");
	                                        print "MDVR diable status Persists";
        	                        else:
                	                        tdkTestObj.setResultStatus("FAILURE");
                        	                print "MDVR disable status does not persist";
