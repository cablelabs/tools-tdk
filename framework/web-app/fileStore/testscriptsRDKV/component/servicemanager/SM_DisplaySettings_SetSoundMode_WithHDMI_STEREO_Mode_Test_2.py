...
<xml>
  ...
  <name>SM_DisplaySettings_SetSoundMode_WithHDMI_STEREO_Mode_Test_2</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test setSoundMode on a connected HDMI port with a valid audio STERIO mode
  ...
  <test_cases>
    <test_case_id>CT_18842_02</test_case_id>
...
"</test_stub_interface>
    <test_script>SM_DisplaySettings_SetSoundMode_WithHDMI_STEREO_Mode_Test_2</test_script>
    <skipped>No</skipped>
    <release_version>M49</release_version>
    <remarks>Set STEREO Mode</remarks>
  </test_cases>
  <script_tags />
</xml>
'''
...
dsObj.configureTestCase(ip,port,'SM_DisplaySettings_SetSoundMode_WithHDMI_STEREO_Mode_Test_2');
...
obj.configureTestCase(ip,port,'SM_DisplaySettings_SetSoundMode_WithHDMI_STERIO_Mode_Test_2');
...
if "SUCCESS" in result.upper():
		...
		if expectedresult in get_ret[0]:
			...
			if "STEREO" in get_ret[1]:
				tdkTestObj.setResultStatus("SUCCESS");
				print "The Audio mode is allready set to STEREO:No need to set the same again"
