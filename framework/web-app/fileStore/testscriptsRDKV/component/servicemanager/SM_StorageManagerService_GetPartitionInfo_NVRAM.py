...
<xml>
  ...
  <name>SM_StorageManagerService_GetPartitionInfo_NVRAM</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getPartitionInfo returns a non empty list of partition information for NVRAM</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_129</test_case_id>
    <test_objective>Checks if the service manager API getPartitionInfo returns a non empty list of partition information for NVRAM</test_objective>
...
CallMethod : const QString - "getPartitionInfo" ,const ServiceParams - bool
...
4. Service_Manager_Agent will invoke "getPartitionInfo" for each partition id present in the device information corresponding to NVRAM.
5. TM will check if the API call is SUCCESS and it returned a non empty list and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the API getPartitionInfo returns a non empty list of partition information.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetPartitionInfo_NVRAM</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetPartitionInfo_NVRAM');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetPartitionInfo_NVRAM');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in iarmLoadStatus.upper():
        ...
        iarmObj.setLoadModuleStatus("SUCCESS");
		...
		#Calling IARM Bus Init
				...
				tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
                                ...
                                if expectedresult in actualresult:
                                        ...
                                        devInfo = json.loads(resultDetails);
                                        print "Device Info : ", devInfo;
					status = False;
					for key in devInfo:
                                                if (devInfo[key].has_key('deviceType') and devInfo[key]['deviceType'] == 4):
							status = True;
							partitionIds = devInfo[key]['partitionIds'];
							for partId in partitionIds:
                                                		print "Device Id: %s Partition Id: %s" %(key,partId);
						                #Call GetPartitionInfo API
				                		tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
						                expectedresult="SUCCESS"
								tdkTestObj.addParameter("service_name", serviceName);
						                tdkTestObj.addParameter("method_name", "getPartitionInfo");
								inputList = [];
								inputList.append(key);
								inputList.append(partId);
								tdkTestObj.addParameter("params", inputList);
									...
									tdkTestObj.setResultStatus("SUCCESS");
					                        	resultDetails = tdkTestObj.getResultDetails();
			               				        partitionInfo = json.loads(resultDetails);
			                		        	print "Partition Info : ", partitionInfo;
									if partitionInfo == []:
										print "Empty PartitionInfo retrieved for", partId;
        	                                                                tdkTestObj.setResultStatus("FAILURE");
								else:
									print "Failed to get PartitionInfo for", partId;
									tdkTestObj.setResultStatus("FAILURE");
									break;
					if not status:
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print "NVRAM details not available in getDeviceInfo list\n";
        			else:
                               		tdkTestObj.setResultStatus("FAILURE");
