...
4.Service_Manager_Agent will invoke the api set_device_name to set the device name.Set the device name multiple times and verify.
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_HN_SetandGet_DeviceName');
...
if "SUCCESS" in smLoadStatus.upper():
        ...
        if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    deviceNameList = ['Test_STB_1','Test_STB_2']
                    for device in range(len(deviceNameList)): 
                       tdkTestObj.addParameter("device_name",deviceNameList[device]);
                       tdkTestObj.executeTestCase(expectedresult);
                       actualresult = tdkTestObj.getResult();
                       deviceNameDetails = tdkTestObj.getResultDetails();
                       print "[TEST EXECUTION DETAILS %s] : %s" %(device+1,deviceNameDetails);
                       if expectedresult in actualresult:
                           #Check whether the device name is set or not
                           if deviceNameList[device] in deviceNameDetails:
                               tdkTestObj.setResultStatus("SUCCESS");
                               print "Successfully set the device name as" ,deviceNameList[device];
                           else:
                               tdkTestObj.setResultStatus("FAILURE");
                               print "Device name is not set as ",deviceNameList[device];
                       else:
                           tdkTestObj.setResultStatus("FAILURE");
