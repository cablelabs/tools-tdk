...
<xml>
  ...
  <name>SM_StorageManagerService_GetDeviceIds</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getDeviceIds returns the list of device ids</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_108</test_case_id>
    <test_objective>Checks if the service manager API getDeviceIds returns the list of device ids</test_objective>
...
CallMethod : const QString - "getDeviceIds" ,const ServiceParams - bool
...
3. On Success of registerService , Service_Manager_Agent will invoke "getDeviceIds" API to get the list of available device ids.
4. Service_Manager_Agent will check if the list is not empty and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the API getDeviceIds returns a non empty list.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetDeviceIds</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetDeviceIds');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetDeviceIds');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in iarmLoadStatus.upper():
		...
		#Calling IARM Bus Init
				...
				tdkTestObj.addParameter("service_name", serviceName);
					...
					if deviceIdList:
						print "Device id list fetched Successfully";
						tdkTestObj.setResultStatus("SUCCESS");
					else:
						print "Device Id list is empty";
