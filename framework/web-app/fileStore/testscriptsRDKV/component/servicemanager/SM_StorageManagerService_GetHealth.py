...
<xml>
  ...
  <name>SM_StorageManagerService_GetHealth</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getHealth returns the NVRAM health status as True</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_133</test_case_id>
    <test_objective>Checks if the service manager API getHealth returns the NVRAM health status as True</test_objective>
...
4. TM will check if the result contains the field 'helathy' and the valure is True and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the NVRAM heath status is True</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetHealth</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetHealth');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetHealth');
		...
		#Calling IARM Bus Init
				...
				tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
							...
							tdkTestObj.addParameter("inputCount", 1);
								...
								print healthInfo;
                		                        	tdkTestObj.setResultStatus("SUCCESS");
								if (healthInfo.has_key('healthy')):
									print "NVRAM health status: ",healthInfo['healthy'];
									if healthInfo['healthy']:
                		                        			tdkTestObj.setResultStatus("SUCCESS");
										print "NVRAM is healthy";
									else:
                		                        			tdkTestObj.setResultStatus("FAILURE");
										print "NVRAM is not healthy";
								else:
									tdkTestObj.setResultStatus("FAILURE");
									print "Result does not contain NVRAM health information";
							else:
								tdkTestObj.setResultStatus("FAILURE");
