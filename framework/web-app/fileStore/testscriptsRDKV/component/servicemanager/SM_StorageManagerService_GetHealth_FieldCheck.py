...
<xml>
  ...
  <name>SM_StorageManagerService_GetHealth_FieldCheck</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the result of service manager API getHealth contains necessary fields</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_134</test_case_id>
    <test_objective>Checks if the result of service manager API getHealth contains necessary fields</test_objective>
...
CallMethod : const QString - "getHealth" ,const ServiceParams - bool
...
3. On Success of registerService , Service_Manager_Agent will invoke "getHealth" API to get the NVRAM health information.
4. TM will check if the result contains the field 'helathy' and 'operational'  and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the NVRAM heath information contains necessary fileds</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetHealth_FieldCheck</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetHealth_FieldCheck');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetHealth_FieldCheck');
		...
		#Calling IARM Bus Init
				...
				tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
                                ...
                                if expectedresult in actualresult:
                                        ...
                                        for key in devInfo:
                                                if (devInfo[key].has_key('deviceType') and devInfo[key]['deviceType'] == 4):
                                                        status = True;
							print "Getting NVRAM health info";
							tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
			                                expectedresult="SUCCESS"
                			                tdkTestObj.addParameter("service_name", serviceName);
                        	        		tdkTestObj.addParameter("method_name", "getHealth");
							tdkTestObj.addParameter("params", key);
							tdkTestObj.addParameter("inputCount", 1);
			                                ...
			                                if expectedresult in actualresult:
                        	        		        resultDetails = tdkTestObj.getResultDetails();
		                	                        healthInfo = json.loads(resultDetails);
								print healthInfo;
                		                        	tdkTestObj.setResultStatus("SUCCESS");
								if (healthInfo.has_key('healthy') and healthInfo.has_key('operational')):
                		                        		tdkTestObj.setResultStatus("SUCCESS");
									print "Result contains necessary fields";
								else:
									tdkTestObj.setResultStatus("FAILURE");
									print "Result does not contain necessary fields";
							else:
								tdkTestObj.setResultStatus("FAILURE");
								print "Failed to get Health Info";
