...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>SM_System_SetandGet_Invalid_Mode</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To try to set an INVALID mode as the STB mode</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_116</test_case_id>
    <test_objective>To set an INVALID mode using system service</test_objective>
...
4.Service_Manager_Agent will invoke the api setMode with mode as INVALID
5.Depending upon the return value of setMode api, SUCCESS/FAILURE status is returned
...
Checkpoint 2.Check the  mode set with setMode by getting it using getMode</except_output>
    ...
    <test_script>SM_System_SetandGet_Invalid_Mode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_SetandGet_Invalid_Mode');
iarmObj.configureTestCase(ip,port,'SM_System_SetandGet_Invalid_Mode');
        ...
        if "SUCCESS" in connect:
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    paramValue = {"duration":20,"mode":"INVALID"}
                    methodName="setMode"
                    ...
                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "SUCCESS:", methodName," call is NOT successful with" ,paramValue;
                        methodDetail = tdkTestObj.getResultDetails();
                        print methodName, ": Details :" ,methodDetail

                    else:
                        print "FAILURE:", methodName, " call is successful with ",paramValue;
			...
			#Reverting to NORMAL Mode
                        ...
                        if expectedresult in actualresult:
                            ...
                            print "FAILURE:", methodName, " call is NOT successful with ",paramValue;
                            tdkTestObj.setResultStatus("FAILURE");
                else:
                    tdkTestObj.setResultStatus("FAILURE");
