...
<xml>
  ...
  <name>SM_StorageManagerService_GetDVRCapacity</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the return value of getDVRCapacity API is valid</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_111</test_case_id>
    <test_objective>Checks if the return value of getDVRCapacity API is valid</test_objective>
...
CallMethod : const QString - "getDVRCapacity" ,const ServiceParams - bool
...
3. On Success of registerService , Service_Manager_Agent will invoke "getDVRCapacity" API to get the value of TSBMaxMinutes.
4. Service_Manager_Agent will check if the value is less than the maximum allowed value and return SUCCESS/FAILURE status.
5. Service_Manager_Agent will de-register the given service from ServiceManager component.</automation_approch>
    <except_output>Checkpoint 1.Check the invocation of the API is success.
Checkpoint 2.Check the return value of getDVRCapacity  API is less than the maximum allowed value.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetDVRCapacity</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetDVRCapacity');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetDVRCapacity');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in iarmLoadStatus.upper():
		...
		#Calling IARM Bus Init
                ...
                if "SUCCESS" in init:
					...
					if dvrCapacity < 500*1024*1024:
						print "DVR Capacity valid";
						tdkTestObj.setResultStatus("SUCCESS");
					else:
						print "DVR Capacity not valid";
                                                tdkTestObj.setResultStatus("FAILURE")
				else:
					tdkTestObj.setResultStatus("FAILURE");
                        		print "Failed to get DVRCapacity \n";
