...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>SM_System_Set_Invalid_Preferred_Standby_Mode</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>7</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set invalid preferred standby mode and check whether it accepts or not</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_75</test_case_id>
    <test_objective>To check whether invalid standby modes are accepted or not</test_objective>
...
4.Service_Manager_Agent will invoke the api setPreferredStandbyMode to set with with invalid value
5.TM will check the values and return SUCCESS/FAILURE status.
...
Checkpoint 2. Invalid standby mode value set should throw an error  .</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_System_Set_Invalid_Preferred_Standby_Mode</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_Set_Invalid_Preferred_Standby_Mode');
iarmObj.configureTestCase(ip,port,'SM_System_Set_Invalid_Preferred_Standby_Mode');
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult:
                        ...
                        print methodName, "call is successful with INVALID parameter",standByMode ;
                        methodDetail = tdkTestObj.getResultDetails();
                        print methodName, ": Details :" ,methodDetail
                    else:
                        print methodName, "call is NOT successful with INVALID parameter" , standByMode;
                        tdkTestObj.setResultStatus("SUCCESS");
                else:
                    tdkTestObj.setResultStatus("FAILURE"); 
