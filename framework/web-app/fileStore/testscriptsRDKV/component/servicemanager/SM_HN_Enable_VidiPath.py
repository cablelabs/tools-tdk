...
<xml>
  ...
  <name>SM_HN_Enable_VidiPath</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if VidiPath status can be enabled using Service manager API</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_59</test_case_id>
    <test_objective>Checks if VidiPath status can be enabled using Service manager API</test_objective>
...
3. On Success of registerService , Service_Manager_Agent will invoke "setVidiPathEnabled" API to set the value of Upnp status to "true".
...
Checkpoint 2. Check the value retrieved using isVidiPathEnabled API is same as the value set using setVidiPathEnabled API.</except_output>
    ...
    <test_script>SM_HN_Enable_VidiPath</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_HN_Enable_VidiPath');
