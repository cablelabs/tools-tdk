...
<xml>
  ...
  <name>SM_StorageManagerService_GetPartitionInfo_HDD</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getPartitionInfo returns a non empty list of partition information for HDD</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Checks if the service manager API getPartitionInfo returns a non empty list of partition information for HDD</test_objective>
...
4. Service_Manager_Agent will invoke "getPartitionInfo" for each partition id present in the device information corresponding to HDD.
...
Checkpoint 2.Check the API getPartitionInfo returns a non empty list of partition information.</except_output>
    ...
    <test_script>SM_StorageManagerService_GetPartitionInfo_HDD</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetPartitionInfo_HDD');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetPartitionInfo_HDD');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in iarmLoadStatus.upper():
		...
		#Calling IARM Bus Init
				...
				tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
					...
					status = False;
					for key in devInfo:
                                                if (devInfo[key].has_key('deviceType') and devInfo[key]['deviceType'] == 0):
							status = True;
					...
					if not status:
                               			tdkTestObj.setResultStatus("FAILURE");
	                                        print "HDD details not available in getDeviceInfo list\n";
