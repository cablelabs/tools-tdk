...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>SM_System_Persist_SetandGet_PowerState_Standby</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>7</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether power state value is getting persisted</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_78</test_case_id>
    <test_objective>To check whether set and get the STANDBY power state using system service persisting or not</test_objective>
...
4.Service_Manager_Agent will invoke the api getAvailableStandbyModes,setPowerState,getPowerState
5.if LIGHT_SLEEP is available then set mode as LIGHT_SLEEP and the power state as STANDBY. Then un register and register the  service.
...
Checkpoint 2.Check the power state is set as STANDBY</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_System_Persist_SetandGet_PowerState_Standby</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_Persist_SetandGet_PowerState_Standby');
iarmObj.configureTestCase(ip,port,'SM_System_Persist_SetandGet_PowerState_Standby');
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult:
                        ...
                        if "LIGHT_SLEEP" in standbyModes:
                            print "LIGHT_SLEEP mode is available"
                            ...
                            if expectedresult in actualresult:
                                tdkTestObj.setResultStatus("SUCCESS");
                                print methodName, "call is successful with parameter",standbyMode ;
                                methodDetail = tdkTestObj.getResultDetails();
                                print methodName, ": Details :" ,methodDetail
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    #Register the system service again
                                    print "Registering the System Service"
                                    serviceName = "systemService";
                                    register = servicemanager.registerService(smObj,serviceName)
                                    if "SUCCESS" in register:
                                        ...
                                        if expectedresult in actualresult:
                                            ...
                                            if powerState==methodDetail:
                                                print "Successfully power state is set as ",powerState ,"with prefered mode" , standbyMode
                                                print "Power state is persisted"
                                                tdkTestObj.setResultStatus("SUCCESS");
                                ...
                                print methodName, "call is NOT successful with parameter" , standbyMode;
                                tdkTestObj.setResultStatus("FAILURE");
                        else:
                            print standbyMode , "mode is NOT available"
                            tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
                            expectedresult="SUCCESS"
                            methodName="setPowerState"
                            powerState="STANDBY"
