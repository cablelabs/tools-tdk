...
<xml>
  ...
  <name>SM_StorageManagerService_GetDVRFreeSpace</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getDVRFreeSpace returns valid value</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_107</test_case_id>
    <test_objective>Checks if the service manager API getDVRFreeSpace returns valid value</test_objective>
...
CallMethod : const QString - "getDVRFreeSpace" ,const ServiceParams - bool
...
3. On Success of registerService , Service_Manager_Agent will invoke "getDVRFreeSpace" API to get the value of TSBFreeSpace.
4. Service_Manager_Agent will invoke "getDVRCapacity" API to get the value of TSBCapacity.
5. Service_Manager_Agent will check if the value of DVRFreeSpace is less than or equal to the value of DVRCapacity and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the value of DVRFreeSpace is less than or equal to the value of DVRCapacity.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetDVRFreeSpace</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetDVRFreeSpace');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetDVRFreeSpace');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in iarmLoadStatus.upper():
		...
		#Calling IARM Bus Init
                ...
                if "SUCCESS" in init:
		                ...
		                tdkTestObj.addParameter("method_name", "getDVRFreeSpace");
                		tdkTestObj.executeTestCase(expectedresult);
		                #Get the result of execution
                		actualresult = tdkTestObj.getResult();
		                if expectedresult in actualresult:
		                        resultDetails = tdkTestObj.getResultDetails();
                		        dvrFreeSpace = json.loads(resultDetails);
		                        print "DVRFreeSpace : ", dvrFreeSpace;
					tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
	                                expectedresult="SUCCESS"
        	                        tdkTestObj.addParameter("service_name", serviceName);
                	                tdkTestObj.addParameter("method_name", "getDVRCapacity");
                        	        tdkTestObj.executeTestCase(expectedresult);
                                	#Get the result of execution
	                                actualresult = tdkTestObj.getResult();
        	                        if expectedresult in actualresult:
                	                        resultDetails = tdkTestObj.getResultDetails();
                        	                dvrCapacity = json.loads(resultDetails);
                                	        print "DVRCapacity : ", dvrCapacity;
						if dvrFreeSpace <= dvrCapacity:
							tdkTestObj.setResultStatus("SUCCESS");
		                                        print "DVRFreeSpace less than DVR Capaciy\n";
						else:
							tdkTestObj.setResultStatus("FAILURE");
		                                        print "DVRFreeSpace is beyond DVR Capaciy\n";
					else:
						tdkTestObj.setResultStatus("FAILURE");
		                                print "Failed to get DVR Capaciy\n";
				else:
					tdkTestObj.setResultStatus("FAILURE");
                        		print "Failed to get DVRFreeSpace \n";
