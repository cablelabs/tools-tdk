...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_79</test_case_id>
    <test_objective>To check whether setting invalid power state throws error</test_objective>
...
4.Service_Manager_Agent will invoke the api setPowerState with invalid value
...
Checkpoint 2.set power state should fail</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_System_Set_Invalid_PowerState</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_Set_Invalid_PowerState');
iarmObj.configureTestCase(ip,port,'SM_System_Set_Invalid_PowerState');
        ...
        if "SUCCESS" in connect:
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("FAILURE");
                        print methodName, "call is successful with INVALID parameter" , powerState;
                        methodDetail = tdkTestObj.getResultDetails();
                        print methodName, ": Details :" ,methodDetail
                    else:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print methodName, "call is NOT successful with INVALID parameter" , powerState;
                else:
                    tdkTestObj.setResultStatus("FAILURE"); 
