...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>SM_System_SetandGet_EAS_NegativeDuration</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set EAS mode with negative duration and verify that mode remains the same till another mode is set.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_112</test_case_id>
    <test_objective>To set EAS mode with negative duration and to verify that the mode is retained using system service</test_objective>
...
Checkpoint 2.Check the  mode set with setMode by getting it using getMode</except_output>
    ...
    <test_script>SM_System_SetandGet_EAS_NegativeDuration</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_SetandGet_EAS_NegativeDuration');
iarmObj.configureTestCase(ip,port,'SM_System_SetandGet_EAS_NegativeDuration');
        ...
        if "SUCCESS" in connect:
            ...
            if "SUCCESS" in register:
                ...
                if expectedresult in actualresult:
                    ...
                    paramValue = {"duration":-1,"mode":"EAS"}
                    methodName="setMode"
                    tdkTestObj.addParameter("service_name", serviceName);
                        ...
                        if expectedresult in actualresult:
                            ...
                            if "EAS" in getModeValue:
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    if "EAS" in getModeValue:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print "Successfully retains the EAS mode"
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print "Failed to retain the EAS mode"
