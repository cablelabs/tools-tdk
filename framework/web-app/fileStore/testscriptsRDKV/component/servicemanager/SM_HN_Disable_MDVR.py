...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if Upnp status can be disabled using Service manager API</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_56</test_case_id>
    <test_objective>Checks if MDVR status can be disabled using Service manager API</test_objective>
...
3. On Success of registerService , Service_Manager_Agent will invoke "setMdvrEnabled" API to set the value of MDVR status to "false".
...
Checkpoint 2. Check the value retrieved using isMdvrEnabled API is same as the value set using setMdvrEnabled API.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_HN_Disable_MDVR</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_HN_Disable_MDVR');
