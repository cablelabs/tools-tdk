...
<xml>
  ...
  <name>SM_StorageManagerService_GetTSBStatus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the service manager API getTSBStatus a valid TSB status code</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_100</test_case_id>
    <test_objective>Checks if the service manager API getTSBStatus a valid TSB status code</test_objective>
...
CallMethod : const QString - "getTSBStatus" ,const ServiceParams - bool
...
3. On Success of registerService , Service_Manager_Agent will invoke "getTSBStatus" API to get the TSB status code.
4. Service_Manager_Agent will check if the status code returned is in the valid list [0,1,2,4,8,16,32,64] and return SUCCESS/FAILURE status.
...
Checkpoint 2.Check the status code returned is valid.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetTSBStatus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetTSBStatus');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetTSBStatus');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in iarmLoadStatus.upper():
		...
		#Calling IARM Bus Init
				...
				statusCodeList = [0,1,2,4,8,16,32,64];
                		tdkTestObj = smObj.createTestStep('SM_Generic_CallMethod');
		                expectedresult="SUCCESS"
                		tdkTestObj.addParameter("service_name", serviceName);
		                tdkTestObj.addParameter("method_name", "getTSBStatus");
					...
					tdkTestObj.setResultStatus("SUCCESS");
                		        ...
                		        tsbStatus = json.loads(resultDetails);
		                        print "TSB Status : ", tsbStatus;
					if tsbStatus in statusCodeList:
						tdkTestObj.setResultStatus("SUCCESS");
                                        	print "Status code valid";
					else:
						tdkTestObj.setResultStatus("FAILURE");
						print "Invalid status code";
				else:
					tdkTestObj.setResultStatus("FAILURE");
                        		print "Failed to get TSB status \n";
