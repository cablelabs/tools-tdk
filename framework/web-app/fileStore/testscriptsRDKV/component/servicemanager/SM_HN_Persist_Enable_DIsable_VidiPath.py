...
<xml>
  ...
  <name>SM_HN_Persist_Enable_DIsable_VidiPath</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>SM_HN_SetVidiPathEnabled</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if VidiPath enable/disable status persists across different instances of service.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_69</test_case_id>
    <test_objective>Checks if VidiPath enable/disable status persists across different instances of service.</test_objective>
...
CallMethod : const QString - "setVidiPathEnabled"/"isVidiPathEnabled" ,const ServiceParams - bool
UnregisterService : Qstring-serviceName</input_parameters>
    <automation_approch>1. TM loads the Service_Manager_Agent via the test agent.
2. Service_Manager_Agent will register "homeNetworkingService" with ServiceManager component.
3. On Success of registerService , Service_Manager_Agent will invoke "setVidiPathEnabled" API to set the value of VidiPath status to "true".
4. Service_Manager_Agent will invoke "isVidiPathEnabled" API to get the value of VidiPath status.
...
8. Service_Manager_Agent will invoke "isVidiPathEnabled" API to get the value of isVidiPathEnabled status.
9. TM will check if the value is "true" and return SUCCESS/FAILURE status.
10. Service_Manager_Agent will invoke "setVidiPathEnabled" API to set the value of VidiPath status to "false".
11. Service_Manager_Agent will invoke "isVidiPathEnabled" API to get the value of VidiPath status.
...
15. Service_Manager_Agent will invoke "isVidiPathEnabled" API to get the value of VidiPath status.
...
Checkpoint 2. Check the enable/disable values persist across different instances of service</except_output>
    ...
    <test_script>SM_HN_Persist_Enable_DIsable_VidiPath</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_HN_Persist_Enable_DIsable_VidiPath');
...
if "SUCCESS" in smLoadStatus.upper():
        ...
        if "SUCCESS" in register:
                ...
                #Enable VidiPath status.
                tdkTestObj = smObj.createTestStep('SM_HN_SetVidiPathEnabled');
                expectedresult = "SUCCESS";
                enable = 1;
                print "Setting VidiPath status to ",enable;
                ...
                #Get VidiPath enable status
                tdkTestObj = smObj.createTestStep('SM_HN_IsVidiPathEnabled');
                expectedresult = "SUCCESS";
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                ...
                if expectedresult in actualresult:
                        ...
                        if enable == int(getEnabledDetails):
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "VidiPath status enabled";
                        else:
                                tdkTestObj.setResultStatus("FAILURE");
                                print "VidiPath status not enabled"
                else:
                        tdkTestObj.setResultStatus("FAILURE");

		#Unregistering and re registering service to check persistence of VidiPath status
			...
			#Get VidiPath enable status
	                ...
	                if expectedresult in actualresult:
                	        ...
                	        if enable == int(getEnabledDetails):
                        	        tdkTestObj.setResultStatus("SUCCESS");
                                	print "VidiPath enable status persists";
	                        else:
        	                        tdkTestObj.setResultStatus("FAILURE");
                	                print "VidiPath enable status does not persist";
	                else:
        	                tdkTestObj.setResultStatus("FAILURE");


	                #Disable VidiPath status.
        	        tdkTestObj = smObj.createTestStep('SM_HN_SetVidiPathEnabled');
                	expectedresult = "SUCCESS";
	                enable = 0;
        	        print "Setting VidiPath status to ",enable;
	                ...
	                if expectedresult in actualresult:
                	        ...
                	        if enable == int(getEnabledDetails):
                        	        tdkTestObj.setResultStatus("SUCCESS");
                                	print "VidiPath status disabled";
	                        else:
        	                        tdkTestObj.setResultStatus("FAILURE");
                	                print "VidiPath status not disabled";
			...
			#Unregistering and re registering service to check persistence of VidiPath status
				...
				#Get VidiPath enable status
                	        ...
                	        if expectedresult in actualresult:
                                	...
                                	if enable == int(getEnabledDetails):
                                        	tdkTestObj.setResultStatus("SUCCESS");
	                                        print "VidiPath disable status persists";
        	                        else:
                	                        tdkTestObj.setResultStatus("FAILURE");
                        	                print "VidiPath disable status does not persist";
