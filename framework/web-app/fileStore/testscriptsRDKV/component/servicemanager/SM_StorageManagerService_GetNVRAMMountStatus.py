...
<xml>
  ...
  <name>SM_StorageManagerService_GetNVRAMMountStatus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Checks if the result of service manager API getDeviceInfo contains contains status field for NVRAM and and the value is '0'</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_135</test_case_id>
    <test_objective>Checks if the result of service manager API getDeviceInfo contains contains status field for NVRAM and and the value is '0'</test_objective>
...
4. TM will check if the result contains status field for NVRAM and and return SUCCESS/FAILURE status based on whether the value is '0' or not.
...
Checkpoint 2.Check the result contains status field for NVRAM and and the value is '0'.</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_StorageManagerService_GetNVRAMMountStatus</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_StorageManagerService_GetNVRAMMountStatus');
iarmObj.configureTestCase(ip,port,'SM_StorageManagerService_GetNVRAMMountStatus');
		...
		#Calling IARM Bus Init
				...
				devInfoList = [];
					...
					for key in devInfoList:
						if devInfoList[key].has_key('deviceType') and devInfoList[key].has_key('status'):
							if devInfoList[key]['deviceType'] == 4:
								status = True;
								if devInfoList[key]['status'] == '0':
									tdkTestObj.setResultStatus("SUCCESS");
									print "NVRAM is mounted successfully";
								else:
									tdkTestObj.setResultStatus("FAILURE");
									print "NVRAM not mounted successfully";
								break;
					if status:
						tdkTestObj.setResultStatus("SUCCESS");
