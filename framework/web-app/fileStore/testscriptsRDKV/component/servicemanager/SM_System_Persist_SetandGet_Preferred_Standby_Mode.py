...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>SM_System_Persist_SetandGet_Preferred_Standby_Mode</name>
  <primitive_test_id/>
  <primitive_test_name>SM_Generic_CallMethod</primitive_test_name>
  <primitive_test_version>7</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check whether Standby Modes are Persists or not</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_74</test_case_id>
    <test_objective>To check whether available stand by modes are persists successfully after set and get .</test_objective>
...
5.Set the available standby modes , unregister the service and register again. Then get the stand by mode.
...
Checkpoint 2.Check whether the standby modes are persisting or not  .</except_output>
    <priority>High</priority>
    <test_stub_interface>libservicemanagerstub.so</test_stub_interface>
    <test_script>SM_System_Persist_SetandGet_Preferred_Standby_Mode</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
smObj.configureTestCase(ip,port,'SM_System_Persist_SetandGet_Preferred_Standby_Mode');
iarmObj.configureTestCase(ip,port,'SM_System_Persist_SetandGet_Preferred_Standby_Mode');
                            ...
                            if "SUCCESS" in register:
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    if standbyMode == standbyModes[mode]:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print "Preferred Standby Mode" , standbyMode, "is persisted";
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
                                        print "Preferred Standby Mode" , standbyMode ,"is NOT persisted";
                                else:
                                    print methodName, "call is NOT successful";
                                    tdkTestObj.setResultStatus("FAILURE");
                            else:
