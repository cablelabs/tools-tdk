...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>SM_HdmiCec_Stress_SendMsg_CecEnabled</name>
  <primitive_test_id>106</primitive_test_id>
  <primitive_test_name>SM_RegisterService</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Objective: Service Manager – Send message to the STB device multiple times (5 times) with enabling CEC.
Test Case Id: CT_Service Manager_42
Test type: Positive.</synopsis>
  ...
  <test_cases>
    <test_case_id>CT_Service Manager_42</test_case_id>
    <test_objective>Service Manager – Send message to the STB device multiple times (5 times) with enabling CEC.</test_objective>
...
4. On Success of enabling cec, Service_Manager_Agent will send message for a CEC device continues 5 times.
...
Checkpoint 2.Check for the message pattern for message sent in cec.txt.
    ...
    <test_script>SM_HdmiCec_Stress_SendMsg_CecEnabled</test_script>
    <skipped>No</skipped>
    <release_version>M25</release_version>
    <remarks>SM_HdmiCec_EnableCec_SendMsgFiveTimes chnged to SM_HdmiCec_Stress_SendMsg_CecEnabled during M-29 release</remarks>
  </test_cases>
</xml>
			...
			if "SUCCESS" in connect:
				...
				tdkTestObj.addParameter("valueToSetEnabled",valueToSetEnabled);
					...
					for i in range(0,5):
        					...
        					expectedresult = "SUCCESS"
        					message = "30 8F 38 95 94 14 " + str(b2a_hex(urandom(1))).upper()
