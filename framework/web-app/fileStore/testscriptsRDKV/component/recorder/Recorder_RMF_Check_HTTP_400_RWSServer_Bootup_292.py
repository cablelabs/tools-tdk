...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>Recorder_RMF_Check_HTTP_400_RWSServer_Bootup_292</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>RWS server should send HTTP 400 when the server is enabled with HTTP 400 error and also during boot up after retry recorder should send error code RDK-10028</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_292</test_case_id>
    <test_objective>RWS server should send HTTP 400 when the server is enabled with HTTP 400 error and also during boot up after retry recorder should send error code RDK-10028</test_objective>
...
2. Configure RWS server and RWS secure server with error 400 and reboot the box
3. Immeadiately after reboot schedule a 2 min legacy recording
4.Check ocapri log for error code 400 and connection retry requests
5. Also check the ocapri log for RDK error code RDK-10028 for connection lost with RWS server.
6.Clear the RWS server and RWS secure server error 400
...
4.TestMgr_Recorder_ExecuteCmd</test_stub_interface>
    <test_script>Recorder_RMF_Check_HTTP_400_RWSServer_Bootup_292</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Check_HTTP_400_RWSServer_Bootup_292');
...
obj.configureTestCase(ip,port,'Recorder_RMF_Check_HTTP_400_RWSServer_Bootup_292');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	loadmoduledetails = recObj.getLoadModuleDetails();
        ...
        actResponse = recorderlib.callServerHandlerWithTypeAndError('enableError','RWSServer',ip,'400');
        print "Enable error :", actResponse
        actResponse = recorderlib.callServerHandlerWithTypeAndError('enableError','RWSSecureServer',ip,'400');
        print "Enable error :", actResponse
        ...
        if "true" in actResponse: 
            ...
            pattern = "Received HTTP response code: 400"
