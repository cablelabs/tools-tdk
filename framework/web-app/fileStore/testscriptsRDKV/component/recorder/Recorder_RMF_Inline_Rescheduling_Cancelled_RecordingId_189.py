...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Inline_Rescheduling_Cancelled_RecordingId_189</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_189 - Recorder should not schedule recording when trying to schedule with a cancelled recording Id</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_189</test_case_id>
    <test_objective>Recorder should not schedule recording when trying to schedule with a cancelled recording Id</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to schedule the current recording and send to TDK Recorder Simulator server which is present in TM.
4.Delete the recording when in progress
5. Again schedule recording with same cancelled recording Id.
6. Reboot the STB after a sleep of recording duration.
7. noUpdate schedule message will be send to TDK Recorder Simulator server once STB is up to get the list of recordings.
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Inline_Rescheduling_Cancelled_RecordingId_189</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Inline_Rescheduling_Cancelled_RecordingId_189');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	sleep(60);
		...
		if 'acknowledgement' not in actResponse:
                    ...
                    jsonMsgRescheduling = "{\"updateSchedule\":{\"requestId\":\""+requestID+"\",\"generationId\":\"TDK123\",\"dvrProtocolVersion\":\"7\",\"schedule\":[{\"recordingId\":\""+recordingID+"\",\"locator\":[\"ocap://"+ocapId+"\"],\"epoch\":"+now+",\"start\":"+startTime+",\"duration\":"+duration2+",\"properties\":{\"requestedStart\":0,\"title\":\"Recording_"+recordingID+"\"},\"bitRate\":\"HIGH_BIT_RATE\",\"deletePriority\":\"P3\"}]}}";
