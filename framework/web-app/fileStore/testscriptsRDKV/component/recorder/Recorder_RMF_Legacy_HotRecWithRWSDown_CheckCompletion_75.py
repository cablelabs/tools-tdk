...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Legacy_HotRecWithRWSDown_CheckCompletion_75</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test that an legacy hot recording scheduled while only RWS is down starts immediately and is eventually playable (even though the ACK for the schedule update was not sent to RWS)</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_75</test_case_id>
    <test_objective>Test that an legacy hot recording scheduled while only RWS is down starts immediately and is eventually playable (even though the ACK for the schedule update was not sent to RWS)</test_objective>
...
4. RecorderAgent / Python lib interface will frame the json message to schedule current recording using legacy mechanism and send to TDK Recorder Simulator server which is present in TM.
5.RecorderAgent / Python lib interface will bring up/activate the RWS server URL
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Legacy_HotRecWithRWSDown_CheckCompletion_75</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Legacy_HotRecWithRWSDown_CheckCompletion_75');
