...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Inline_SchedRecWithRWSDown_CheckRecStatus_186</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>If Recorder encounters failure sending any message to RWS, recording status of future inline recording that have not been sent need to be re-sent once RWS connectivity is re-established</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_186</test_case_id>
    <test_objective>Check that recording status of future inline recording that have not been sent due to RWS connection failure, need to be re-sent once RWS connectivity is re-established</test_objective>
...
3.RecorderAgent / Python lib interface will down the RWS status server URL 
4.RecorderAgent / Python lib interface will frame the json message to schedule a future recording using inline mechanism and send to TDK Recorder Simulator server which is present in TM.
5. RecorderAgent / Python lib interface will bring up the RWS status server URL after recording completed
...
Checkpoint 2 Get the response from recorder and verify that status has been received in full sync</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Inline_SchedRecWithRWSDown_CheckRecStatus_186</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Inline_SchedRecWithRWSDown_CheckRecStatus_186');
        ...
        if "FALSE" in status.upper():
                ...
                if "updateSchedule" in serverResponse:
                        ...
                        if "FALSE" in status.upper():
                                ...
                                print "Wait for 60 sconds to get response from recorder"
				sleep(60);
