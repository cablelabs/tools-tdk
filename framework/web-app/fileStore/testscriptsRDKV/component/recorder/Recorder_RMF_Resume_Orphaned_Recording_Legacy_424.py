...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>Recorder_RMF_Resume_Orphaned_Recording_Legacy_424</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Orphaned recordings should resume after notifying to RWS provided end time is not expired</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_424</test_case_id>
    <test_objective>Recorder should resume orphaned recordings using legacy after reboot provided end time is not expired</test_objective>
...
2.Schedule a  future recording of 11 minute duration  using  legacy
3.Wait for the recording to complete partially and delete the meta data (Including backup of Blobs and XML)
4.Reboot the box
5.Check the status of recording, it should be ERASED and error should  set as ORPHANED.
6.Sent recording update for the orphaned recording since the recording end time has not expired.
7.Recording should resume and it should have status as INCOMPLETE with error STARTED_LATE
...
2.TestMgr_Recorder_DeleteRecordingMetaData</test_stub_interface>
    <test_script>Recorder_RMF_Resume_Orphaned_Recording_Legacy_424</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Resume_Orphaned_Recording_Legacy_424');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	print "Full sync response: %s" %actResponse;
        ...
        #Since DVR simulator will not send full schedule after reboot sending the updateSchedule from script
	jsonMsg = "{\"updateSchedule\":{\"requestId\":\""+requestID+"\",\"generationId\":\""+genIdInput+"\",\"fullSchedule\":false,\"dvrProtocolVersion\":\"7\",\"schedule\":[{\"recordingId\":\""+ recordingID+"\",\"locator\":[\"ocap://"+ocapId+"\"],\"epoch\":"+epoch_time+",\"start\":"+startTime+",\"duration\":"+duration+",\"properties\":{\"requestedStart\":0,\"title\":\"Recording_"+recordingID+"\"},\"bitRate\":\"HIGH_BIT_RATE\",\"deletePriority\":\"P3\"}]}}"
	...
	print "Wait for the recording to resume and complete"
	sleep(200); 
	...
	actResponse = recorderlib.callServerHandler('retrieveStatus',ip)
        ...
        if 'NOTFOUND' not in recordingData:
            ...
            if "INCOMPLETE" in statusValue.upper() and "STARTED_LATE" in errorValue.upper():
                print "Recording has status INCOMPLETE and error STARTED_LATE"
                tdkTestObj.setResultStatus("SUCCESS");
            else:
                tdkTestObj.setResultStatus("FAILURE");
                print "Recording has unexpected values"
