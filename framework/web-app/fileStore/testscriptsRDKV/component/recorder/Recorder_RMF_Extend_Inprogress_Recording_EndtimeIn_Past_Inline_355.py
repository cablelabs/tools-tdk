...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Extend_Inprogress_Recording_EndtimeIn_Past_Inline_355</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Extend an inprogress recording with end time in past</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_355</test_case_id>
    <test_objective>To extend an inprogress recording with end time in past using inline</test_objective>
...
3.Schedule a future recording of 5 minutes duration with start time  2 minutes using inline
4.After 30 sec , extend the duration of  recording to a past end time using updateSchedule message 
5. Wait for the recording to complete
6. Check whether the expected duration of  recording is NOT extended and also check the actual duration of recording.
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Extend_Inprogress_Recording_EndtimeIn_Past_Inline_355</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Extend_Inprogress_Recording_EndtimeIn_Past_Inline_355');
	...
	sleep(10);
		...
		print "Retrieve Status Details: %s"%actResponse;
                if 'acknowledgement' in actResponse:
                    ...
                    #Extend duration to a past end time
                    epoch_time = str(int(epoch_time) - 600000)
			...
			print "Retrieve Status Details: %s"%actResponse;
				...
				if "INCOMPLETE" in statusValue.upper():
                                	if (int(newDuration) == int(expDurationValue) and actDurationValue[0] < 60000):
                                    		tdkTestObj.setResultStatus("SUCCESS");
                                    		print "Recording NOT extended to a past end time";
					else:
						tdkTestObj.setResultStatus("FAILURE");
						print "Recording extended to a past end time";
                                else:
                                    	tdkTestObj.setResultStatus("FAILURE");
