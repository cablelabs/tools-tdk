...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_145</test_case_id>
    <test_objective>Recorder should not send redundant recording statuses; new status only needs to be sent if number of playable segments has changed or status has changed (change in Error code only does not require status to be re-sent) - Checking by cancelling the future recording</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to schedule the future recording and send to TDK Recorder Simulator server which is present in TM.
4. Schedule another recording to cancel the future recording which in progress using inline mechanism.
5. STB will be rebooted after a sleep of recording duration.
6. Recorder should not send the same response again
...
Checkpoint 2 Verify the list of recordings to check the state of the cancelled recording Id.</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Verify_RedundantStatus_Cancel_FutureRecording_145</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Verify_RedundantStatus_Cancel_FutureRecording_145');
	...
	sleep(10);
		...
		print "Retrieve Status Details: %s"%actResponse;
		    ...
		    response = recorderlib.callServerHandler('clearStatus',ip);
                    ...
                    jsonMsgCancelRecording = "{\"updateSchedule\":{\"requestId\":\""+requestID+"\",\"generationId\":\"TDK123\",\"fullSchedule\":true,\"cancelRecordings\":[\""+recordingID+"\"]}}";
                    expResponse = "updateSchedule";
			...
			print "Retrieve Status Details: %s"%actResponse;
				...
				if "USER_STOP" in error.upper() and "ERASED" in status.upper():
                                	...
                                	tdkTestObj.setResultStatus("FAILURE");
                                	print "Scheduled recording error not received correctly";
			    else:
                                    tdkTestObj.setResultStatus("FAILURE");
