...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Inline_TRM_Cancel_Same_OcapId_50</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_50 - Recorder not to send error TRM_CANCELLED if current anf future recording is scheduled with same ocap id via inline</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_50</test_case_id>
    <test_objective>Recorder to send error TRM_CANCELLED if future recording is cancelled by TRM message via inline</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to schedule the current and future recording for the same ocap ID as an inline message and send to TDK Recorder Simulator server which is present in TM.
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Inline_TRM_Cancel_Same_OcapId_50</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Inline_TRM_Cancel_Same_OcapId_50');
	...
	sleep(60);
		...
		if 'acknowledgement' not in actResponse:
                    ...
                    print "Received Empty/Error status";
                        ...
                        actResponse = recorderlib.callServerHandler('retrieveStatus',ip);
                        print actResponse;
                        recordingData = recorderlib.getRecordingFromRecId(actResponse,str(int(recordingID)+1))
                        print recordingData
