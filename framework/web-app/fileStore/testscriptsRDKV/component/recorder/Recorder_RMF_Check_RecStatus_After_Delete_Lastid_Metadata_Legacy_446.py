...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Check_RecStatus_After_Delete_Lastid_Metadata_Legacy_446</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check whether recording details are persisting when lastid.dat file and meta data (including backup )are missing</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_446</test_case_id>
    <test_objective>Recorder should persist the legacyrecording details when lastid.dat file and meta data (including backup )are corrupted</test_objective>
...
2.Schedule a hot legacy recording of 2 minutes duration.
3.Wait for the recording to complete and check whether recording completed successfully.
4..Delete the lastid.dat file and meta data of the  recording.(including delete .bak files)
5.Schedule one more current recording of 2 minutes
6.Wait for the recording to complete.
7.Reboot STB.
8.Check the status of first recording, it should be ERASED and error should be set to ORPHANED.
9.Depends on the result of above step RecorderAgent sends SUCCESS or FAILURE to TM.
</automation_approch>
...
2.TestMgr_Recorder_ExecuteCmd</test_stub_interface>
    <test_script>Recorder_RMF_Check_RecStatus_After_Delete_Lastid_Metadata_Legacy_446</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Check_RecStatus_After_Delete_Lastid_Metadata_Legacy_446');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	print "Recording marked as COMPLETE ";
        ...
        print "Deleting lastid.dat file"
        tdkTestObj1 = recObj.createTestStep('Recorder_ExecuteCmd');
        expectedResult="SUCCESS";
        tdkTestObj1.addParameter("command","rm /tmp/mnt/diska3/persistent/dvr/recdbser/lastid.dat");
        #Execute the test case in STB
        tdkTestObj1.executeTestCase("SUCCESS");
        result = tdkTestObj1.getResult();
        print "[TEST EXECUTION RESULT] : %s" %result;
