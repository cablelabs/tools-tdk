...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Cancel_Failed_Rec_Inline_441</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Recorder should not change the status when cancelling a recording which is end with FAILED</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_441</test_case_id>
    <test_objective>Recorder should not change the status when cancelling a legacy recording which is end with status FAILED</test_objective>
...
2.Schedule a hot inline recording of 1 minute duration using invalid ocap value
3.Check whether the recording is FAILED or not.
4.Sent json message to cancel the recording
5.Check whether recorder has changed the status of recording from FAILED to some other status
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Cancel_Failed_Rec_Inline_441</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Cancel_Failed_Rec_Inline_441');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	sleep(10);
        ...
        response = recorderlib.callServerHandler('clearStatus',ip);
        #Frame json message for update recording^M
        ...
        if 'NOTFOUND' not in recordingData:
            ...
            if "FAILED" in status.upper():
                tdkTestObj.setResultStatus("SUCCESS");
                print "FAILED Status NOT changed";
            else:
                tdkTestObj.setResultStatus("FAILURE");
                print "FAILED status changed";
        else:
            tdkTestObj.setResultStatus("SUCCESS");
            print "Failed recordings are immeadiately purged from box";
