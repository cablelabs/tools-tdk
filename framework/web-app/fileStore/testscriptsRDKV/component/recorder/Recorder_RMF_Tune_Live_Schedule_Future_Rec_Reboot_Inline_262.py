...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Tune_Live_Schedule_Future_Rec_Reboot_Inline_262</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_262 -Recordings should always transition to "InComplete" Not "Started" if it has started with status "StartedWithError"</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_262</test_case_id>
    <test_objective>check whether recordings should always transition to "StartedInComplete" Not "Started" if it has started with status "StartedWithError" using inline</test_objective>
...
4.RecorderAgent / Python lib interface will frame the json message to schedule a  future recording of 2 min duration with  using inline mechanism and send to TDK Recorder Simulator server which is present in TM.
5.Rebbot the box before recording starts
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Tune_Live_Schedule_Future_Rec_Reboot_Inline_262</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Tune_Live_Schedule_Future_Rec_Reboot_Inline_262');
                    ...
                    if 'acknowledgement' not in actResponse:
                        ...
                        if 'NOTFOUND' not in recordingData:
                            ...
                            if "INCOMPLETE" in statusValue.upper():
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "Recording status set to INCOMPLETE successfully.";
                            else:
                                tdkTestObj.setResultStatus("FAILURE");
...
if Expected_Result in loadModuleStatus.upper():
        ...
        print "unloading mediaframework module";
