...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Update_SourceID_Multiple_Inline_419</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Multiple Source ID change for a recording in the schedule update should reflect in the box</synopsis>
  <groups_id/>
...
3.Send a future schedule for two recordings with same start time and with same source id using inline
4.Immediately after that using update schedule msg with fullschedule as false send a different but valid source id change for one of the recordings using inline.
5.Again update the source id with a valid one using inline.
6.Both recordings should record successfully.
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Update_SourceID_Multiple_Inline_419</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Update_SourceID_Multiple_Inline_419');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	sleep(10);
		...
		if 'acknowledgement' not in actResponse:
                    ...
                    jsonMsgUpdateSchedule1 = "{\"updateSchedule\":{\"requestId\":\""+requestID+"\",\"generationId\":\"TDK123\",\"fullSchedule\":false,\"dvrProtocolVersion\":\"7\",\"schedule\":[{\"recordingId\":\""+recordingID+"\",\"locator\":[\"ocap://"+ocapId2+"\"]}]}}";
                    expResponse = "updateSchedule";
                    tdkTestObj.executeTestCase(expectedResult);
                    actResponse = recorderlib.callServerHandlerWithMsg('updateInlineMessage',jsonMsgUpdateSchedule1,ip);

                    sleep(30);
                    #Frame json message for update recording
                    jsonMsgUpdateSchedule2 = "{\"updateSchedule\":{\"requestId\":\""+requestID+"\",\"generationId\":\"TDK123\",\"fullSchedule\":false,\"dvrProtocolVersion\":\"7\",\"schedule\":[{\"recordingId\":\""+recordingID+"\",\"locator\":[\"ocap://"+ocapId3+"\"]}]}}";

                    expResponse = "updateSchedule";
                    tdkTestObj.executeTestCase(expectedResult);
                    actResponse = recorderlib.callServerHandlerWithMsg('updateInlineMessage',jsonMsgUpdateSchedule2,ip);
