...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>Recorder_RMF_Check_StatusOf_Erased_Recording_After_Reboot_Legacy_267</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_267 - To check the status of a recording which is moved to p0 priority after reboot</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_267</test_case_id>
    <test_objective>To check the status of a recording which is moved to p0 priority after reboot using legacy</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to schedule a   recording with p4 priority  using legacy mechanism and send to TDK Recorder Simulator server which is present in TM.
4.Update the priority to P0
5.reboot the box and getrecordings message will be send to TDK Recorder Simulator server once STB is up to get the list of recordings.
6. Verify the completed recording
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Check_StatusOf_Erased_Recording_After_Reboot_Legacy_267</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Check_StatusOf_Erased_Recording_After_Reboot_Legacy_267');
                ...
                if 'acknowledgement' not in actResponse:
                    ...
                    if expResponse in actResponse:
                        ...
                        if 'acknowledgement' not in actResponse:
                            ...
                            print recordingData
                            print "Recording details should be there untill a space requirement arises ";
                            #Expecting there is no space requirement arisesd from box
                            if 'NOTFOUND' in recordingData:
                                tdkTestObj1.setResultStatus("FAILURE");
                                print "Recording purged completely from the box ";
                            else:
                                statusKey = 'status'
