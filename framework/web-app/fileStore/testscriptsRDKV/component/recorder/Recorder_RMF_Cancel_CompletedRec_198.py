...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Cancel_CompletedRec_198</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_198 - Recorder not to send Complete status followed by Incomplete when a completed recording is cancelled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_198</test_case_id>
    <test_objective>Recorder not to send Complete status followed by Incomplete when a completed recording is cancelled.</test_objective>
...
4. Once the recording is completed, do a cancel recording for the completed recording Id.
5. Reboot the STB.
6. noUpdate schedule message will be send to TDK Recorder Simulator server once STB is up to get the list of recordings.
7. Recorder to send COMPLETE status and not Incomplete/Failed
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Cancel_CompletedRec_198</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Cancel_CompletedRec_198');
	...
	sleep(60);
		...
		if 'acknowledgement' not in actResponse:
			...
			if 'acknowledgement' not in actResponse:
				...
				if "COMPLETE" in statusValue.upper() and "INCOMPLETE" not in statusValue.upper():
                                	tdkTestObj.setResultStatus("SUCCESS");
                                	print "Scheduled recording status received successfully";
                                elif "BADVALUE" in value.upper():
                                        tdkTestObj.setResultStatus("FAILURE");
                                	print "No status field in recording status";
                            	else:
                                	tdkTestObj.setResultStatus("FAILURE");
                                	print "Scheduled recording error not received";
			    else:
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print "No message from recorder for cancelling a completed recording";	
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
