...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Extend_Durationof_InComplete_Recording_Legacy_342</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Recorder should not extend the duration of an Incomplete recording due to power interrupt</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_342</test_case_id>
    <test_objective>Recorder should not extend the duration of an Incomplete recording due to power interrupt using legacy</test_objective>
...
3.Schedule a hot recording of 11minutes duration using legacy
4.Wait for the recording to complete partially and reboot the box
5. After recording end time got expired extend the duration to 13 minutes
6. Check whether the expected duration of  recording is NOT extended and also check the actual duration of recoridng.
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Extend_Durationof_InComplete_Recording_Legacy_342</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Extend_Durationof_InComplete_Recording_Legacy_342');
		...
		print "Retrieve Status Details: %s"%actResponse;
			...
			actResponse = recorderlib.callServerHandler('retrieveStatus',ip);
				...
				if "INCOMPLETE" in statusValue.upper():
                                	if (int(duration) == int(expDurationValue) and actDurationValue < 500000):
                                    		tdkTestObj.setResultStatus("SUCCESS");
                                    		print "Recorder NOT updated the duration of an incomplete recording";
					else:
						tdkTestObj.setResultStatus("FAILURE");
						print "Recorder updated the duration of an incomplete recording";
                                else:
                                    	tdkTestObj.setResultStatus("FAILURE");
