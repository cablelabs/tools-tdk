...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_15</test_case_id>
    <test_objective>Recorder- To accept Generation ID via updateRecordings retrieved from RWS</test_objective>
...
3.RecorderAgent/Python lib interface will frame the json message to Simulate a legacy 
  longpoll notification (no inline payload, only RWS GET URL) and send to TDK Recorder Simulator server which 
  is present in TM.
4.When Recorder makes a GET request to RWS, simulate an RWS "updateRecordings" response bearing HTTP header 
  "X-Parker-Generation-ID: test2b" through RecorderAgent/Python lib interface.
5.STB (Recorder) makes a POST request to RWS bearing HTTP header "X-Parker-Generation-ID: test2b" with "acknowledgement" payload.(CP)
...
9.STB (Recorder) makes a GET request to RWS bearing HTTP header "X-Parker-Generation-ID: test2b" (CP)</automation_approch>
    <except_output>Checkpoint : Acknowledgement status from the DVRSimulator AND value of generationId from http header at step 5 and 9</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_GenerationId_UpdateRecording_RWS_15</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
