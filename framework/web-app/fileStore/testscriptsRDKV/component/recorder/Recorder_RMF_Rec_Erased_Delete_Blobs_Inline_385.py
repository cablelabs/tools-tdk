...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Rec_Erased_Delete_Blobs_Inline_385</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check the status of a completed recording after deleting it's blobs</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_385</test_case_id>
    <test_objective>Recordings with missing blobs should report as Status=Erased, Error=ORPHANED in the next full sync using inline</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to schedule the current recording of 2 minutes using inline
  and send to TDK Recorder Simulator server which is present in TM.
4.Wait for the recording to complete.
5.Check the status of the scheduled recording, it should be complete.                                                                                            9. 6.Delete the blobs of previously completed recording. (Delete .bak files also)                                  
7.RecorderAgent / Python lib interface will frame the json message to schedule the one more current recording of 2 minutes using inline
  and send to TDK Recorder Simulator server which is present in TM.
8.Wait for the recording to complete.
9.Reboot STB.
10.noUpdate schedule message will be send to TDK Recorder Simulator server once STB is up to get the list of recordings.
11.Check the status of first recording, it should be ERASED and error should be set to ORPHANED.                             
12.Status of the Json response from Recorder to TDK Recorder Simulator server getting extracted by TM.
13.Depends on the result of above step RecorderAgent sends SUCCESS or FAILURE to TM.</automation_approch>
...
2.TestMgr_Recorder_ExecuteCmd</test_stub_interface>
    <test_script>Recorder_RMF_Rec_Erased_Delete_Blobs_Inline_385</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Rec_Erased_Delete_Blobs_Inline_385');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	if "ERASED" not in value.upper():
       		tdkTestObj.setResultStatus("FAILURE");
	        print "Recording not marked as ERASED when it's blobs are  missing";
        	recObj.unloadModule("Recorder");
		exit();
	print "Recording marked as ERASED when it's blobs are missing";
 	key = 'error'
        value = recorderlib.getValueFromKeyInRecording(recordingData,key)
	print "key: ",key," value: ",value
        if "" == value.upper():
	...
	if "ORPHANED" not in value.upper():
       		tdkTestObj.setResultStatus("FAILURE");
	        print "error not marked as ORPHANED when it's blobs are missing";
        	recObj.unloadModule("Recorder");
		exit();
	print "error marked as ORPHANED when it's blobs are missing";
