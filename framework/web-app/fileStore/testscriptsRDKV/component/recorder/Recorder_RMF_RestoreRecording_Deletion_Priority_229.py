...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_RestoreRecording_Deletion_Priority_229</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_229 - Recorder- To update deletion priority from p4 to p1 and restoring it upon receiving updateRecordings message</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_229</test_case_id>
    <test_objective>To Restore a recording after deleting upon receiving updateRecordings message and checking the recording should not have undesirable values.</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to update deletePriority field and restore the deletePriority field for a specified recording Id with generation id = 0 and send to TDK Recorder Simulator server which is present in TM.
...
Checkpoint 2 Get the list of recordings to check whether the recording fields not have undesirable values.</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_RestoreRecording_Deletion_Priority_229</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_RestoreRecording_Deletion_Priority_229');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	sleep(10);
		...
		if 'acknowledgement' not in actResponse:
                    ...
                    if expResponse in actResponse:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "updateRecordings message post success1";
                        #Check for acknowledgement from recorder
                        tdkTestObj.executeTestCase(expectedResult);
                        print "Looping till acknowledgement is received"
                        loop = 0;
			...
			if 'acknowledgement' not in actResponse:
                            tdkTestObj.setResultStatus("FAILURE");
                            print "Received Empty/Error status1";
                        elif 'acknowledgement' in actResponse:
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "Successfully retrieved acknowledgement from recorder1";
                            sleep(60);
			    ...
			    expResponse = "updateRecordings";
			        ...
			        if 'acknowledgement' not in actResponse:
                                    ...
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print "Successfully retrieved acknowledgement from recorder2";
                                    sleep(60);
                                    ...
                                    if 'NOTFOUND' not in recordingData:
                                        ...
                                        reqRecording = {"recordingId":recordingID,"duration":60000,"deletePriority":"P4"}
