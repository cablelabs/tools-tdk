...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_FutureRecording_Extend_Duration_After_Reboot_Inline_333</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Schedule a future recording then extend the duration after reboot and check the recording end time is the extended value</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_333</test_case_id>
    <test_objective>To extend the duration of a future recording after reboot using inline</test_objective>
...
3.Schedule a a future recording of 11 minutes duration with start time 1 minute using inline
4.After recording starts Reboot the box 
5. Extend the duration of recording by 13 minutes using updateSchedule message and Wait for the recording to complete
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_FutureRecording_Extend_Duration_After_Reboot_Inline_333</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_FutureRecording_Extend_Duration_After_Reboot_Inline_333');
			...
			actResponse = recorderlib.callServerHandler('retrieveStatus',ip);
				...
				if "INCOMPLETE" in statusValue.upper():
                                	if (int(newDuration) == int(expDurationValue) and actDurationValue > 500000):
                                    		tdkTestObj.setResultStatus("SUCCESS");
                                    		print "Using Update Schedule message duration updated for a future recording after reboot";
					else:
						tdkTestObj.setResultStatus("FAILURE");
						print "Using Update Schedule message duration NOT updated for a future recording after reboot";
                                else:
                                    	tdkTestObj.setResultStatus("FAILURE");
