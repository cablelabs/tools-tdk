...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_HotRec_Legacy_NoStatus_106</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>35. Recorder should send no status message at all if hot recording legacy starts and ends on time without error</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_106</test_case_id>
    <test_objective>Check that recorder not sending any status message if a legacy hot recording starts and ends on time without error</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to schedule a current recording using legacy mechanism and send to TDK Recorder Simulator server which is present in TM.
4. Wait till recording is completed.
...
Checkpoint 2 Get the response from recorder and verify that no status sent for the recording from recorder.</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_HotRec_Legacy_NoStatus_106</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
obj.configureTestCase(ip,port,'Recorder_RMF_HotRec_Legacy_NoStatus_106');
        ...
        if "updateSchedule" in serverResponse:
                ...
                if "acknowledgement" in recResponse:
                        ...
                        if ('NOTFOUND' not in recordingData):
                            ...
                            print "Recorder not sent any status for the recording which starts and ends without error";
                else:
                        tdkTestObj.setResultStatus("FAILURE");
