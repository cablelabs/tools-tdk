...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Verify_RepeatStatus_DeleteRecording_InProgress_149</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_149 - Recorder should not send repeats of the same status - verifying by cancelling the in progress recording and then deleting the cancelled recording and checking the status</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_149</test_case_id>
    <test_objective>Recorder should not send repeats of the same status - verifying by cancelling the in progress recording and then deleting the cancelled recording and checking the status</test_objective>
...
3.RecorderAgent / Python lib interface will frame  Delete Recording json message send to TDK Recorder Simulator server which uses inline mechanism to send to Recorder.
4. STB will be rebooted after a sleep of recording duration.
5. CHeck for acknowledgement received on RWS Post server and check it is not sent again
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Verify_RepeatStatus_DeleteRecording_InProgress_149</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Verify_RepeatStatus_DeleteRecording_InProgress_149');
		...
		if 'acknowledgement' not in actResponse:
			...
			if 'acknowledgement' not in actResponse:
				...
				if "INCOMPLETE" in value.upper():
                                	print "Recording INCOMPLETE status received successfully";
                                	response = recorderlib.callServerHandler('clearStatus',ip);
