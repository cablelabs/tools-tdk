...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_CancelRecording_Nearendtime_ScheduleRecording_Inline_280</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Cancel a recording near the end time of recording and schedule another recording and it should get start.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_280</test_case_id>
    <test_objective>Cancel a recording near the end time of recording and schedule another recording and it should get start using inline</test_objective>
...
3.Schedule a recording and cancel it near the end timeof recording
4.Check whether recording cancelled succesfully
5.Schedule another recording
6.getrecordings message will be send to TDK Recorder Simulator server once STB is up to get the list of recordings.
7Check the newly scheduled recording is started or not
...
Checkpoint 2 Check whether recording cancelled succesfully
Checkpoint 3 Check the newly scheduled recording is started or not</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_CancelRecording_Nearendtime_ScheduleRecording_Inline_280</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_CancelRecording_Nearendtime_ScheduleRecording_Inline_280');
                                        ...
                                        if expResponse in actResponse:
                                            ...
                                            if 'acknowledgement' not in actResponse:
                                                ...
                                                if 'NOTFOUND' not in recordingData:
                                                    ...
                                                    tdkTestObj.setResultStatus("SUCCESS");
                                                    if "STARTED" in statusValue.upper():
                                                        tdkTestObj.setResultStatus("SUCCESS");
                                                        print "Recording status set to STARTED";
                                                    else:
                                                        tdkTestObj.setResultStatus("FAILURE");
                                                        print "Recording status NOT set to STARTED";
                                                else:
                                                    tdkTestObj.setResultStatus("FAILURE");
