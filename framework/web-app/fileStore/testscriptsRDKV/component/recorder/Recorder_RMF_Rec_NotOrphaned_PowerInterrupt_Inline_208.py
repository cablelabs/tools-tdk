...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>Recorder_RMF_Rec_NotOrphaned_PowerInterrupt_Inline_208</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_208</test_case_id>
    <test_objective>Check that recording interrupted by set top reboot and becomes corrupted and lost, 
recorder resumes recording what’s left and report as u'POWER_INTERRUPTION. It should not be reported as ORPHANED.</test_objective>
...
3.RecorderAgent/Python lib interface will frame the json message to schedule the current recording of 20 minutes and send to TDK Recorder Simulator server which is present in TM.
4.Wait for the recording to complete partially.
5.Delete the meta data of the ongoing recording.(dont delete .bak files)
6.Reboot STB.
7.Wait for the recording to complete.
8.Reboot STB.
9.noUpdate schedule message will be send to TDK Recorder Simulator server once STB is up to get the list of recordings.
10.Check the status of the recording, it should be INCOMPLETE and error should be set to POWER_INTERRUPTION.
...
12.Depends on the result of above step RecorderAgent sends SUCCESS or FAILURE to TM."</automation_approch>
    <except_output>Checkpoint Get the response from recorder and verify that status has been received in full sync after second reboot for the recording and verify the expected status and error.</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_DeleteRecordingMetaData
2.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Rec_NotOrphaned_PowerInterrupt_Inline_208</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Rec_NotOrphaned_PowerInterrupt_Inline_208');
