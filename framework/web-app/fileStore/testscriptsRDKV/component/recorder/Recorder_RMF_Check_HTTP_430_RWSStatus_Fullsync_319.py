...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>Recorder_RMF_Check_HTTP_430_RWSStatus_Fullsync_319</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>RWS status server should send HTTP 430 when the server is enabled with HTTP 430 error and also during full sync after retry recorder should send error code RDK-10028</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recorder_DVR_Protocol_319</test_case_id>
    <test_objective>RWS status server should send HTTP 430 when the server is enabled with HTTP 430 error and also during full sync after retry recorder should send error code RDK-10028</test_objective>
...
3.Configure RWS status server and RWS secure status server with error 430 and reboot the box
 4.Check ocapri log and application log for error code 430 and connection retry requests
5. Also check the ocapri log for RDK error code RDK-10028 for connection lost with RWS server.
6.Clear the RWS status server and RWS secure status server error 430
...
4.TestMgr_Recorder_ExecuteCmd</test_stub_interface>
    <test_script>Recorder_RMF_Check_HTTP_430_RWSStatus_Fullsync_319</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Check_HTTP_430_RWSStatus_Fullsync_319');
...
obj.configureTestCase(ip,port,'Recorder_RMF_Check_HTTP_430_RWSStatus_Fullsync_319');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	loadmoduledetails = recObj.getLoadModuleDetails();
        ...
        actResponse = recorderlib.callServerHandlerWithTypeAndError('enableError','RWSStatus',ip,'430');
        print "Enable error :", actResponse
        actResponse = recorderlib.callServerHandlerWithTypeAndError('enableError','RWSSecureStatus',ip,'430');
        print "Enable error :", actResponse
        ...
        if "true" in actResponse:
            ...
            tdkTestObj2.addParameter("command","(cat /opt/logs/applications.log; cat /opt/logs/ocapri_log.txt) | grep \"HTTP/1.1 430\"");
            #Execute the test case in STB
            ...
            if result:
                ...
                print "HTTP 430 error received ";
            else:
                tdkTestObj2.setResultStatus("FAILURE");
                print "HTTP 430 error NOT received ";
