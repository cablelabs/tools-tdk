...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Rescheduling_Same_RecordingId_Erased_Check_38</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_38- Recorder-To support cancelling of a recording when rescheduling it with the same Recording ID. Check whether status is not pending/erased</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_38</test_case_id>
    <test_objective>Recorder-To support cancelling of a recording when rescheduling it with the same Recording ID. Check whether status is not pending/erased</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to schedule the current recording with generation id = 0 and send to TDK Recorder Simulator server which is present in TM.
4. Reschedule will be done with the same recording Id .
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Rescheduling_Same_RecordingId_Erased_Check_38</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Rescheduling_Same_RecordingId_Erased_Check_38');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	sleep(60);
		...
		if 'acknowledgement' not in actResponse:
                    ...
                    jsonMsgRescheduling = "{\"updateSchedule\":{\"requestId\":\""+requestID+"\",\"generationId\":\"TDK123\",\"dvrProtocolVersion\":\"7\",\"schedule\":[{\"recordingId\":\""+recordingID+"\",\"locator\":[\"ocap://"+ocapId+"\"],\"epoch\":"+now+",\"start\":"+startTime+",\"duration\":"+duration+",\"properties\":{\"requestedStart\":0,\"title\":\"Recording_"+recordingID+"\"},\"bitRate\":\"HIGH_BIT_RATE\",\"deletePriority\":\"P3\"}]}}";

                    expResponse = "updateSchedule";
                    tdkTestObj.executeTestCase(expectedResult);
                    actResponse = recorderlib.callServerHandlerWithMsg('updateMessage',jsonMsgRescheduling,ip);
