...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>Recorder_RMF_Delete_P0_Recording_Inline_237</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>After deleting a recording checking the availability of recording metadata in /tmp/mnt/diska3/persistent/dvr/recdbser</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_237</test_case_id>
    <test_objective>To check the presence of recording metadata after deleting that particular recording.(inline)</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to schedule a recording and update deletePriority field of that recording Id with generation id = 0 using inline mechanism and send to TDK Recorder Simulator server which is present in TM.
4.After that check the presence of recoridng metadata in /tmp/mnt/diska3/persistent/dvr/recdbser
...
Checkpoint 2 The recording metadata should still be available in inside ‘/tmp/mnt/diska3/persistent/dvr/recdbuser</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_SendRequest
2.TestMgr_Recorder_PresenceOfRecordingMetaData</test_stub_interface>
    <test_script>Recorder_RMF_Delete_P0_Recording_Inline_237</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Delete_P0_Recording');
	...
	sleep(60);
		...
		if 'acknowledgement' not in actResponse:
			...
			if 'acknowledgement' not in actResponse:
                            ...
                            print result,",Metadata of Recording ",recordingID,":",details
                            if "FAILURE" in result:
                                print "Metadata for the Recording : ",recordingID,"having priority P0 is not available at location /tmp/mnt/diska3/persistent/dvr/recdbser"
                                tdkTestObj2.setResultStatus("FAILURE");
                            else:
                                print "Metadata for the Recording : ",recordingID,"having priority P0 is available at location /tmp/mnt/diska3/persistent/dvr/recdbser"
