...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_Verify_Ack_Inline_UpdateRecording_MultipleRec_96</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_96 - Recorder should respond to updateRecordings messages, whether received from longpoll by sending an “acknowledgement” message to RWS.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_96</test_case_id>
    <test_objective>Recorder should respond to updateRecordings messages, whether received from longpoll by sending an âacknowledgementâ</test_objective>
...
3.RecorderAgent / Python lib interface will frame multiple updateRecordings json message and send to TDK Recorder Simulator server which uses inline mechanism to send to Recorder.
4. STB will be rebooted after a sleep of recording duration.
5. CHeck for acknowledgement received on RWS Post server
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_Verify_Ack_Inline_UpdateRecording_MultipleRec_96</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_Verify_Ack_Inline_UpdateRecording_MultipleRec_96');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	sleep(60);
		...
		while loop < 5:
	                ...
	                print "Update Schedule Retrieve Status Details: %s"%actResponse;
			sleep(10);
			loop = loop+1;
		if 'acknowledgement' not in actResponse:
                    ...
                    response = recorderlib.callServerHandler('clearStatus',ip);
                    print "Clear Status Details after update Schedule Ack: %s"%response;

                    #Frame json message for update recording
                    jsonMsgUpdateRecording = "{\"updateRecordings\":{\"requestId\":\""+requestID+"\",\"generationId\":\"0\",\"dvrProtocolVersion\":\"7\",\"recordings\":[{\"recordingId\":\""+recordingID+"\",\"deletePriority\":\"P2\"},{\"recordingId\":\""+str(int(recordingID)+1)+"\",\"deletePriority\":\"P2\"}]}}";
                    ...
                    if expResponse in actResponse:
                        ...
                        while loop < 5:
                                actResponse = recorderlib.callServerHandler('retrieveStatus',ip);
                                print "Update Recording Retrieve Status Details: %s"%actResponse;
                                sleep(10);
                                loop = loop+1;
			if 'acknowledgement' not in actResponse:
                            tdkTestObj.setResultStatus("FAILURE");
