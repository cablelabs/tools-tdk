...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_WITH_REQUESTED_START_Inline_176</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_176</test_case_id>
    <test_objective>Recorder must not send Error=MISSING_REQUESTED_START when requestedStart is sent in JSON request</test_objective>
...
3.Frame the inline json message with requestedStart in it through RecorderAgent/Python lib interface to schedule the 
  current recording of 5 minutes and send to TDK Recorder Simulator server which is present in TM.
4.Wait for the recording duration.
5.Reboot the STB.
...
Checkpoint 2 : Get the response from recorder in full sync and verify that status has been set to complete.</except_output>
    <priority>High</priority>
    <test_stub_interface>RecorderAgent
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_WITH_REQUESTED_START_Inline_176</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_WITH_REQUESTED_START_Inline_176');
...
if "SUCCESS" in recLoadStatus.upper():
	...
	# Causing rmfStreamer crash
	#jsonMsg = "{\"updateSchedule\":{\"requestId\":\""+requestID+"\",\"generationId\":\""+genIdInput+"\",\"dvrProtocolVersion\":\"7\",\"schedule\":[{\"recordingId\":\""+ recordingID+"\",\"locator\":[\"ocap://"+ocapId+"\"],\"epoch\":"+now+",\"start\":"+"0"+",\"duration\":"+duration+",\"properties\":{\"requestedStart\":"+now+",\"title\":\"Recording_"+recordingID+"\"},\"bitRate\":\"HIGH_BIT_RATE\",\"deletePriority\":\"P3\"}]}}"
