...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_EOR_StartedLate_EndedOnTime_Legacy_128</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Recorder must always send an end-of-recording status if it ever sent any start-of-recording status.For example if it starts late but ends on time, then Recorder still needs to send Incomplete at the end.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_128</test_case_id>
    <test_objective>check that Recorder sends an end-of-recording status if it ever sent any start-of-recording status for recording started late and ended on time.</test_objective>
...
3.Frame the legacy json message through RecorderAgent/Python lib interface to schedule the future (starting after 90 seconds) recording 
  of 5 minutes and send to TDK Recorder Simulator server which is present in TM.
4.Reboot the STB so that recording starts late
4.Once the STB and re4corder is up wait for the recording to complete.
5.Reboot the STB.
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_EOR_StartedLate_EndedOnTime_Legacy_128</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_EOR_StartedLate_EndedOnTime_Legacy_128');
