...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>Recorder_RMF_UpdateRecording_Legacy_LongPoll_InvalidJson_40</name>
  <primitive_test_id/>
  <primitive_test_name>Recorder_SendRequest</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>CT_Recoder_DVR_Protocol_40 - Recorder-No to accept updateRecording messages via legacy style	 for invalid json message</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>CT_Recoder_DVR_Protocol_40</test_case_id>
    <test_objective>Recorder-No to accept updateRecording messages via legacy style     for invalid json message</test_objective>
...
3.RecorderAgent / Python lib interface will frame the json message to update deletePriority field for a specified recording Id with errors in the JSON mesage request and send to TDK Recorder Simulator server which is present in TM.
...
1.TestMgr_Recorder_SendRequest</test_stub_interface>
    <test_script>Recorder_RMF_UpdateRecording_Legacy_LongPoll_InvalidJson_40</test_script>
    <skipped>Yes</skipped>
    <release_version>This script is removed during M31 because Recorder does not send acknowledgement back to RWS now as per BEI-106</release_version>
    <remarks>This script is removed during M31 because Recorder does not send acknowledgement back to RWS now as per BEI-106</remarks>
  </test_cases>
</xml>
...
recObj.configureTestCase(ip,port,'Recorder_RMF_UpdateRecording_Legacy_LongPoll_InvalidJson_40');
	...
	sleep(60);
		...
		if 'acknowledgement' not in actResponse:
			...
			if 'acknowledgement' not in actResponse:
                            ...
                            tdkTestObj.setResultStatus("FAILURE");
                            print "Retrieved acknowledgement from recorder";
			    ...
			    print "Recording List: %s" %actResponse;
                            ...
                            if 'NOTFOUND' not in recordingData:
                                ...
                                if "P2" not in value.upper():
                                    tdkTestObj1.setResultStatus("SUCCESS");
                                    print "update recording not happened because of invalid json";
                                else:
                                    tdkTestObj1.setResultStatus("FAILURE");
                                    print "update recording happened with invalid json message";
                            else:
                                tdkTestObj1.setResultStatus("FAILURE");
                    ...
                    tdkTestObj.setResultStatus("FAILURE");
                    print "Failed to retrieve empty status from recorder";
        else:
            tdkTestObj.setResultStatus("FAILURE");
