...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1516</id>
  <version>5</version>
  <name>E2E_RMF_MDVR_PausePlay_SameRecord</name>
  <primitive_test_id>583</primitive_test_id>
  <primitive_test_name>TDKE2E_MDVR_GetResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify that 2 client boxes [XI3] connected to an XG1 box can perform pause on one client box and playback the same recorded content on another client box simultaneously without any issues. CT_MDVR_3</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>E2E_MDVR_14</test_case_id>
    <test_objective>To verify that 2 client boxes [XI3] connected to an XG1 box can perform pause on one client box and playback the same recorded content on another client box simultaneously without any issues.</test_objective>
...
3) TM frames the request urls, one recordingId from list of recorded urls and passes it to XI3 agents simultaneously  record pause in one clint box  and  playback the same  recorded content in another client box to pause through mplayer.
4) TM reads Success or Failure message from each connected client and sends final result.</automation_approch>
    ...
    <test_script>E2E_RMF_MDVR_PausePlay_SameRecord</test_script>
    <skipped>No</skipped>
...
globalObj.configureTestCase(ip,port,'E2E_RMF_MDVR_TrickPlay_SameRecord');
...
if "SUCCESS" in result.upper():
    ...
    if (clientsCount >= 2):
        ...
        # Request for playing same recorded content on 2 clients
