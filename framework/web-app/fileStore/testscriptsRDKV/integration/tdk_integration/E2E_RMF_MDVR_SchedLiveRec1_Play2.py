...
Test Case Id: CT_MDVR_11</synopsis>
  ...
  <test_cases>
    <test_case_id>E2E_MDVR_11</test_case_id>
    <test_objective>To verify if one client box can schedule request for recording live channel while another client box just play a an already recorded content</test_objective>
...
3) TM frames a request url for live recording from first XI3 to XG1 to schedule the agent on XG1 to record the DVR recording.
4) TM frames a request url to playback recorded content in step3 on second XI3.
5) TM reads the log file to extract each recorded url and send it to the XI3 agent to play through mplayer.</automation_approch>
    ...
    <test_script>E2E_RMF_MDVR_SchedLiveRec1_Play2</test_script>
    <skipped>Yes</skipped>
