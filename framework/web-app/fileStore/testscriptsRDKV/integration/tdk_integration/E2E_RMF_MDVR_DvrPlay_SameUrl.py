...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <status>FREE</status>
  <synopsis>To verify that 2 client boxes [XI3] can playback same recorded content on an XG1 box simultaneously without any issues.
Test Case ID : CT_MDVR_02</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>E2E_MDVR_02</test_case_id>
    <test_objective>To verify that 2 client boxes [XI3] can playback same recorded content on an XG1 box simultaneously without any issues.</test_objective>
...
3) TM frames a request url with a recordingId from list of recorded urls and passes it to XI3 agents simultaneously to play through mplayer.
4) TM reads Success or Failure message from each connected client and sends final result.</automation_approch>
    ...
    <test_script>E2E_RMF_MDVR_DvrPlay_SameUrl</test_script>
    <skipped>No</skipped>
...
def TDKE2E_mDVR_PlayUrl(IP,PORT,args=(),kwargs={}):
   ...
   obj.configureTestCase(ip,port,'E2E_RMF_MDVR_DvrPlay_SameUrl');
...
globalObj.configureTestCase(ip,port,'E2E_RMF_MDVR_DvrPlay_SameUrl');
