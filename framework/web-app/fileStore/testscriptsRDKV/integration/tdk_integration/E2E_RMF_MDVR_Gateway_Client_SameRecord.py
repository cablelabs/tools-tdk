...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <status>FREE</status>
  <synopsis>To verify that when XG1 playback the recorded content  try to playback the same recorded content in 2 client boxes from XG1.  CT_MDVR_11</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>E2E_MDVR_18</test_case_id>
    <test_objective>To verify that when XG1 playback the recorded content  try to playback the same recorded content in 2 client boxes from XG1</test_objective>
...
4) TM reads Success or Failure message from each connected client and sends final result.</automation_approch>
    ...
    <test_script>E2E_RMF_MDVR_Gateway_Client_SameRecord</test_script>
    <skipped>No</skipped>
...
def TDKE2E_mDVR_PlayUrl(IP,PORT,args=(),kwargs={}):
   ...
   obj.configureTestCase(ip,port,'E2E_RMF_MDVR_Gateway_Client_SameRecord');
...
globalObj.configureTestCase(ip,port,'E2E_RMF_MDVR_Gateway_Client_SameRecord');
