...
<xml>
  ...
  <name>E2E_RMF_MDVR_Max_ScheduleRecording_Neg</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify that more than 3 client boxes  [XI3] cannot schedule recordings on a single XG1 box simultaneously.
Test Case Id: CT_MDVR_08</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
def TDKE2E_Schedule_Recording(IP,PORT,args=(),kwargs={}):
   ...
   obj.configureTestCase(ip,port,'E2E_RMF_MDVR_Max_ScheduleRecording_Neg');
...
globalObj.configureTestCase(ip,port,'E2E_RMF_MDVR_Max_ScheduleRecording_Neg');
...
if "SUCCESS" in result.upper():
    ...
    if (clientsCount < 4):
        details = "Pre-Condition not met. Connect four client boxes"
