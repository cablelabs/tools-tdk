...
2. Change the power setting to STANDBY using IARMBUS
3.TM gets an source_id from the streaming details page of the FW and sends it to Recorder_agent to generate request url.
4.TM pass the parameters like duration and recording_id to Recorder_agent.
5.Recorder_agent will frame the json message to schedule the recording and send to TDK_Recorder_server which is present in TM.
6.Status of the Json response from Mediastreamer to TDK_Recorder_Server getting extracted by TM.
7 Recorder_agent  will do the error checking by verifying ocapri_logs.
...
Checkpoint 2 Verifying the ocapRI_log to check the state of Recording.</except_output>
    ...
    <test_script>E2E_RMF_standbymode_beforeRecording</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
