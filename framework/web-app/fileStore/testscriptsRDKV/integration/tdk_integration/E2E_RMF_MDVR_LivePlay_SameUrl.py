...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1286</id>
  <version>1</version>
  <name>E2E_RMF_MDVR_LivePlay_SameUrl</name>
  <primitive_test_id>583</primitive_test_id>
  <primitive_test_name>TDKE2E_MDVR_GetResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify that 2 client boxes [XI3] can perform live trickplay of same channel on a single XG1 box simultaneously without any issues.
Test Case ID : CT_MDVR_04</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>E2E_MDVR_04</test_case_id>
    <test_objective>To verify that 2 client boxes [XI3] can perform live trickplay of same channel on a single XG1 box simultaneously without any issues.</test_objective>
...
4) TM reads Success or Failure message from each connected client and sends final result.</automation_approch>
    ...
    <test_script>E2E_RMF_MDVR_LivePlay_SameUrl</test_script>
    <skipped>No</skipped>
...
def TDKE2E_mDVR_PlayUrl(IP,PORT,args=(),kwargs={}):
   ...
   obj.configureTestCase(ip,port,'E2E_RMF_MDVR_LivePlay_SameUrl');
...
globalObj.configureTestCase(ip,port,'E2E_RMF_MDVR_LivePlay_SameUrl');
...
if "SUCCESS" in result.upper():
    ...
    if (clientsCount >= 2):
        ...
        # Request for live trickplay of same stream on 2 clients
        ...
        URL = tdkintegration.E2E_getStreamingURL(globalObj, "LIVE" , streamDetails.getGatewayIp() , streamDetails.getOCAPID());
        if URL == "NULL":
