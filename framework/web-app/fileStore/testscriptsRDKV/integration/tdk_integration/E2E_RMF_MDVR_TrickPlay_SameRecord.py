...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1515</id>
  <version>12</version>
  <name>E2E_RMF_MDVR_TrickPlay_SameRecord</name>
  <primitive_test_id>583</primitive_test_id>
  <primitive_test_name>TDKE2E_MDVR_GetResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify that 2 client boxes [XI3] can perform trick play on the  same recorded content on an XG1 box simultaneously without any issues. CT_MDVR_2</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>E2E_MDVR_13</test_case_id>
    <test_objective>To verify that 2 client boxes [XI3] can perform trick play on the  same recorded content on an XG1 box simultaneously without any issues.</test_objective>
...
3) TM frames request urls, recordingId from list of recorded urls and passes it to XI3 agents simultaneously to play through mplayer.
4) TM reads Success or Failure message from each connected client and sends final result.</automation_approch>
    ...
    <test_script>E2E_RMF_MDVR_TrickPlay_SameRecord</test_script>
    <skipped>No</skipped>
...
def TDKE2E_mDVR_PlayUrl(IP,PORT,args=(),kwargs={}):
   ...
   obj.configureTestCase(ip,port,'E2E_RMF_MDVR_TrickPlay_SameRecord');
   ...
   if "SUCCESS" in result.upper():
      ...
      globalstreamDetails = tdkTestObj.getStreamDetails('01');
