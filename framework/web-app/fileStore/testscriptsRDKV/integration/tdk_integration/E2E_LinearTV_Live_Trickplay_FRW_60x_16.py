...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id>1153</id>
  <version>1</version>
  <name>E2E_LinearTV_Live_Trickplay_FRW_60x_16</name>
  <primitive_test_id>575</primitive_test_id>
  <primitive_test_name>TDKE2E_LinearTV_T2p_Tuning</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This script tests the playback of fast reverse with play speed -60x in LinearTV trickplay scenario Test Case ID : E2E_LinearTV_16</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>E2E_LinearTV_LiveTrickplay_16</test_case_id>
    <test_objective>LinearTV – To verify trickplay rates of 60x than normal stream speed in reverse directions for playback .</test_objective>
...
3.LinearTV_agent will get ocapId and tunes to that ocapId through T2p message. Upon receiving the T2p response the agent should extract the status and send to TM.                    
4.TM passes the playback rate to the LinearTV_agent.
5. LinearTV_agent creates the t2p message request and sends it to video proxy. Upon receiving the t2p message response the LinearTV_agent extracts Success or Failure message and sends it to TM.</automation_approch>
    <except_output>Checkpoint 1. Verifying the response of t2p message from the video proxy for successful playback trick play with “60x”  reverse direction. T2p message will have the response message as rate=-60, status=”OK” and statusMessage=”Trick Mode request successful”.</except_output>
    <priority>Medium</priority>
    <test_stub_interface>TDKIntegration_Stub</test_stub_interface>
    <test_script>E2E_LinearTV_Live_Trickplay_FRW_60x_16</test_script>
    <skipped>No</skipped>
