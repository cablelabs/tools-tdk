...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To release sink DTCP-IP packet. The processed packet may contain DTCP Manager allocated/owned buffers/memory - this call frees up these resources. 
TestType: Positive
TestCaseID: CT_DTCP_28</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'DTCP_ReleaseSinkPacket_28');
...
if "SUCCESS" in loadmodulestatus.upper():
  ...
  dtcp.startSource(tdkTestObj,expectedresult,kwargs={'ifName':'lo','port':5016})
  result = tdkTestObj.getResult();
  if "SUCCESS" in result:
        #Disable unique key exchange
        dtcp.createSinkSession(tdkTestObj,expectedresult,kwargs={'srcIp':'127.0.0.1','srcPort':5016,'uniqueKey':0,'maxPacketSize':4096})
