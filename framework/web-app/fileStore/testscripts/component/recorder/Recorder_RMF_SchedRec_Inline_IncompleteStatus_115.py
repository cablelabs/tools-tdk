...
<xml>
  ...
  <name>Recorder_RMF_SchedRec_Inline_IncompleteStatus_115</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Recorder should send Incomplete notification if the recording(future, inline) has been interrupted by another recording before the first recording completed.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
obj.configureTestCase(ip,port,'Recorder_RMF_SchedRec_Inline_IncompleteStatus_115');
	...
	loadmoduledetails = obj.getLoadModuleDetails();
        ...
        if "updateSchedule" in serverResponse:
			...
			startTime2 = "0";
                        ...
                        #Frame json message
                        RequestURL2 = "{\"updateSchedule\":{\"requestId\":\""+requestID2+"\",\"generationId\":\""+genIdInput+"\",\"dvrProtocolVersion\":\"7\",\"schedule\":[{\"recordingId\":\""+recordingID2+"\",\"locator\":[\"ocap://"+ocapId+"\"],\"epoch\":"+now+",\"start\":"+startTime2+",\"duration\":"+duration+",\"properties\":{\"title\":\"Recording_"+recordingID2+"\"},\"bitRate\":\"HIGH_BIT_RATE\",\"deletePriority\":\"P3\"}]}}";
                        serverResponse2 = recorderlib.callServerHandlerWithMsg('updateInlineMessage',RequestURL2,ip);
                        print "serverResponse2 : %s" %serverResponse2;
                        if "updateSchedule" in serverResponse2:
                                ...
                                retry = 0;
                                while (( ([] == recResponse2) or ('acknowledgement' not in recResponse2) ) and (retry < 10 )):
                                        sleep(10);
                                        recResponse2 = recorderlib.callServerHandler('retrieveStatus',ip);
                                        retry += 1
                                print "Retrieve Status Details for recording 2: ",recResponse2;
                                print "Wait till recording 2 end";
                                sleep(180);
                                actResponse = recorderlib.callServerHandler('retrieveStatus',ip);
                                print "Retrieve Status Details after end of recoring 2:",actResponse;
