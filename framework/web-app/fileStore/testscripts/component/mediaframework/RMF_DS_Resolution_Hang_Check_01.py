...
<xml>
  ...
  <name>RMF_DS_Resolution_Hang_Check_01</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check the defect:Pace XG1-DELIA-[VBN_master]: Macroblocking screen for 10 to 12 seconds. show up during changing resolution from '1080p60 HD'to 480 or 720
TCID: CT_RMF_DEFECTS_01
Note:This script will return duplicates, If running second time without restarting agent. Agent process may lead to crash/restart.This is an issue with DS.</synopsis>
  ...
  <remarks>This testcase is causing TDK agent to crash. Tracking ticket no: CGRTS-425.</remarks>
...
mediaframework_obj.configureTestCase(ip,port,'RMF_DS_Resolution_Defect_Check_01');
ds_obj.configureTestCase(ip,port,'RMF_DS_Resolution_Hang_Check_01');
...
def Create_and_ExecuteTestStep(teststep, testobject, expectedresult,parametername, parametervalue):
    ...
    if teststep == "RMF_Element_Open":
        ...
        url = 'http://' + streamDetails.getGatewayIp() + ':8080/vldms/tuner?ocap_locator=ocap://'+streamDetails.getOCAPID()+'&tsb=60';
