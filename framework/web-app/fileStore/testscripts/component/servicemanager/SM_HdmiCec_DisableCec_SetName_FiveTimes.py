...
<xml>
  ...
  <name>SM_HdmiCec_DisableCec_SetName_FiveTimes</name>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'SM_HdmiCec_DisableCec_SetName_FiveTimes');
iarm_obj.configureTestCase(ip,port,'SM_HdmiCec_DisableCec_SetName_FiveTimes');
...
if expected_Result in loadModuleStatus.upper():
        ...
        if expectedresult in actualresult:
				...
				tdkTestObj.addParameter("valueToSetEnabled",valueToSetEnabled);
					...
					i = 0
					setNameList = ["tdk_hdmicec_01","tdk_hdmicec_02","tdk_hdmicec_03","tdk_hdmicec_04","tdk_hdmicec_05"]
					while i < len(setNameList): 			
						
						#Set the device Name.
		                                tdkTestObj = obj.createTestStep('SM_HdmiCec_SetName');
	        	                        expectedresult = "SUCCESS"
	                	                nameToSet = setNameList[i]
						print "Name to be set: ",nameToSet
						...
						if expectedresult in actualresult:
							...
							print "setName FAILURE on "+str(i)+" time."
						i = i + 1						
				else:
					tdkTestObj.setResultStatus("FAILURE");
