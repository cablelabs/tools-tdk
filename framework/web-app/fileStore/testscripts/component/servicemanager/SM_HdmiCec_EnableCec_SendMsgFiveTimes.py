...
<xml>
  ...
  <name>SM_HdmiCec_EnableCec_SendMsgFiveTimes</name>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'SM_HdmiCec_EnableCec_SendMsgFiveTimes');
iarm_obj.configureTestCase(ip,port,'SM_HdmiCec_EnableCec_SendMsgFiveTimes');
...
def sendMsg(obj):
	...
	#Clear the Cec.txt and send msg
        time.sleep(15)
...
if expected_Result in loadModuleStatus.upper():
        ...
        if expectedresult in actualresult:
				...
				tdkTestObj.addParameter("valueToSetEnabled",valueToSetEnabled);
					...
					#Send the message number of times.
					sendMsgsNum = 5
					i = 0
					while i < sendMsgsNum:	
						ret = sendMsg(obj)
