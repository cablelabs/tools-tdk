...
<xml>
  ...
  <name>SM_HdmiCec_GetDefaultCecDevice_Name</name>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'SM_HdmiCec_GetDefaultCecDevice_Name');
iarm_obj.configureTestCase(ip,port,'SM_HdmiCec_GetDefaultCecDevice_Name');
                ...
                if expectedresult in actualresult:
                        if "PRESENT" in existdetails:
                                ...
                                if expectedresult in actualresult:
                                	...
                                	#Get the cec enabled default device name.
	                                tdkTestObj = obj.createTestStep('SM_HdmiCec_GetName');
        	                        expectedresult = "SUCCESS"
                	                tdkTestObj.executeTestCase(expectedresult);
                        	        actualresult = tdkTestObj.getResult();
	                                getNameDetials = tdkTestObj.getResultDetails();
        	                        print "[TEST EXECUTION DETAILS] : ",getNameDetials;

                	                if expectedresult in actualresult:

                        	                #deafult name = STB
                                	        defaultName = "STB"
                                        	valueGetName = getNameDetials[5:]
