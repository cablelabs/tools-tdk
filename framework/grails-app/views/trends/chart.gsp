...
<script type="text/javascript">
	$(document).ready(
			function() {
				...
				$('input[name=chartOption][value=ExecutionBased]').prop(
						'checked', true);
				$("#previous").hide();
				$("#next").hide();
				$("#home").hide();
	...
	<div>
		...
		<g:if test="${category == "RDKV"}">
			<div>
				...
				&emsp;&emsp;&emsp;&emsp; <input onclick="showExecutionBased();"
					type="radio" name="chartOption" value="ExecutionBased"
					checked="checked" />Compare Results by Execution Name &emsp;<input
					onclick="showDeviceBased();" type="radio" name="chartOption"
					value="DeviceBased" />Compare Results by Device Details
		...
		<g:else>
			<script>
				alert("Performance data not included in RDKB ");
			</script>
		</g:else>
