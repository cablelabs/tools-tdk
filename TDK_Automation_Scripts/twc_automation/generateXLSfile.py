...
HEADER=["testCasename","testCaseDescription","componentName","testSuiteName","testType","testFnName","testFnDescription","testFnType","FinalReadingValue","FinalReadingUnits","relevanceNum",BUILD_VER]
LENGTH_OF_HEADER=len(HEADER)
TEST_SUMMARY_HEADER=["ComponentName","TotalTestSuiteCount","TotalTestCount","TestPassCount","TestFailCount","OtherTestCount", "TestExecutionTime", "STB_RDK_Version" , "STB_TDK_Version","Date Of execution","Execution Name"]
...
def generateXLSFile(filesPath, fileName, relevanceNumber):
	...
	fileExists=False
 	TEST_REPORT_NAME=str(componentName);
	...
	for testFn in testFunctions:
		...
		if relevanceNum >= relevanceNumber and ( testCasename not in testCaseListInXLS or testFnName not in testFnListInXLS or testFnDescription not in testFnDescListInXLS):
			...
			TESTLIST=[]
			TESTLIST.extend((testCasename , testCaseDescription , componentName , testSuiteName , testType , testFnName , testFnDescription ,  testFnType  ,  FinalReadingValue , FinalReadingUnits , str(relevanceNum) ,FinalpassFailStatus))
			WriteToXlsSheet(TESTLIST[:])

#Function to append the result for all successive Build versions in Report
def appendbuildversion(file_path, BUILD_VER, incidentType, testCaseName, testFunctionName):
	...
	print "Number of columns: ", TESTS_PassFail_STATUS_SHEET.ncols
	print "\nNumber of rows: ", TESTS_PassFail_STATUS_SHEET.nrows

	if BUILD_VER not in rl:
		for column in range(0,TESTS_PassFail_STATUS_SHEET.ncols - 1):
			columnCount+=1
			if(columnCount==TESTS_PassFail_STATUS_SHEET.ncols-1):
				w_sheet1.write(0,TESTS_PassFail_STATUS_SHEET.ncols,BUILD_VER)	
		...
		for row in range (1,TESTS_PassFail_STATUS_SHEET.nrows):
			if(TESTS_PassFail_STATUS_SHEET.cell_value(row,0)==testCaseName and TESTS_PassFail_STATUS_SHEET.cell_value(row,5)==testFunctionName):
				print "FunctionName Matched: ",TESTS_PassFail_STATUS_SHEET.cell_value(row,5),"==",testFunctionName
				w_sheet1.write(row,TESTS_PassFail_STATUS_SHEET.ncols,incidentType)
	else:
		print"Build version is already present."
...
def WriteToXlsSheet(TESTLIST):
	...
	if(os.path.exists(file_path)):
		...
		appendbuildversion(file_path, BUILD_VER , incidentType, testCaseName, testFunctionName)
	else:
...
if len(sys.argv) == 2:
	...
	for file in sorted(os.listdir(logsfolder)):
		if file.endswith("_xmllog.xml") and not file.endswith("invalid_xmllog.xml") :
			...
			print "---excluded--- "+file
	print "Final Test Report Name : " + TEST_REPORT_NAME + "_TestReport.xls"
	...
	book.save(TEST_REPORT_NAME+"_TestReport.xls")
