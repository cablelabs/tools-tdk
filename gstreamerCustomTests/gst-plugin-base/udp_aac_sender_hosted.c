...
static gboolean evtHandler (GstBus *bus, GstMessage *msg, gpointer data)
{
  ...
  switch (GST_MESSAGE_TYPE (msg)) {
    ...
    case GST_MESSAGE_ERROR: {
      ...
      /* Parse the error message received */
...
/* Check input arguments */
  ...
  rtppay= gst_element_factory_make ("rtppcmapay", "rtppay");
  udpsink = gst_element_factory_make ("udpsink", "udpsink");



  if (!pipeline || !filesrc || !rtppay || !udpsink)
{
 ...
 /* we add all elements into the pipeline */
  gst_bin_add_many (GST_BIN (pipeline), filesrc, rtppay, udpsink, NULL);
  g_print ("Added all the Elements into the pipeline\n");


  /* we link the elements together */
  gst_element_link_many (filesrc, rtppay, udpsink, NULL);
