...
bool DeviceSettingsAgent::AOP_setLevel(IN const Json::Value& req, OUT Json::Value& response)
{
	...
	try
	{
		...
		sprintf(levelDetails,"%f",audio_level);
...
bool DeviceSettingsAgent::VOP_isDisplayConnected(IN const Json::Value& req, OUT Json::Value& response)
{
	...
	std::string portName=req["port_name"].asCString();
	/*getting instance for video ports*/
	char displayDetails1[30] ="DisplayConnection Status :";
	...
	try
	{
		...
		{
			sprintf(displayDetails,"%s","TRUE");
			response["result"]= "SUCCESS"; 
		}
		else if(display_connect==false)
		{
			sprintf(displayDetails,"%s","FALSE");
			response["result"]= "SUCCESS"; 
		}
		else
		{
		...
		strcat(displayDetails1,displayDetails);
...
bool DeviceSettingsAgent::VOPTYPE_HDCPSupport(IN const Json::Value& req, OUT Json::Value& response)
{
	...
	char HDCPSupportDetails1[20] ="HDCP set Status :";
	char *HDCPSupportDetails = (char*)malloc(sizeof(char)*20);
	memset(HDCPSupportDetails,'\0', (sizeof(char)*20));
	try
	{
		/*getting instance for video ports*/	
		...
		if(HDCPEnable==true)
		{
			sprintf(HDCPSupportDetails,"%s","TRUE");
			response["result"]= "SUCCESS"; 
		}
		else if(HDCPEnable==false)
		{
			sprintf(HDCPSupportDetails,"%s","FALSE");
			response["result"]= "SUCCESS"; 
		}
		else
		{
		...
		strcat(HDCPSupportDetails1,HDCPSupportDetails);
...
bool DeviceSettingsAgent::VOPTYPE_DTCPSupport(IN const Json::Value& req, OUT Json::Value& response)
{
	...
	int portID=req["port_id"].asInt();
	char DTCPSupportDetails1[20] ="DTCP set Status :";
	bool DTCPEnable=false;
	char *DTCPSupportDetails = (char*)malloc(sizeof(char)*20);
	memset(DTCPSupportDetails,'\0', (sizeof(char)*20));
	try
	{
		/*getting instance for video ports*/	
		device::VideoOutputPort vPort = device::Host::getInstance().getVideoOutputPort(portID);
		...
		if(DTCPEnable==true)
		{
			sprintf(DTCPSupportDetails,"%s","TRUE");
			response["result"]= "SUCCESS"; 
		}
		else if(DTCPEnable==false)
		{
			sprintf(DTCPSupportDetails,"%s","FALSE");
			response["result"]= "SUCCESS"; 
		}
		else
		{
		...
		strcat(DTCPSupportDetails1,DTCPSupportDetails);
