					...
					if(deviceInstance){
						...
						if(deviceInstance?.boxType?.name.equals(boxType.trim())){
							...
							if(scriptGroup){
								...
								//Category category = scriptGroup?.category
										...
										if(!executionNameForCheck){
											...
											try {
												...
												executionSaveStatus = executionService.saveExecutionDetails(execName,[scriptName:scriptname, deviceName:deviceName, scriptGroupInstance:scriptGroup,
													appUrl:url, isBenchMark:isBenchMark1, isSystemDiagnostics:isSystemDiagnostics1, rerun:rerun1, isLogReqd:isLogReqd1,category:category.toString()])
					...
					if(scriptStatus && scriptVersionStatus){
						if(!executionNameForCheck){
							...
							if(scriptName.equals(MULTIPLESCRIPT)){
								...
								executionSaveStatus = executionService.saveExecutionDetailsOnMultipleScripts(execName, scriptName, deviceName, scriptGroupInstance,url,isBenchMark,isSystemDiagnostics,rerun,isLogReqd,scriptCount, params?.category)
							}else{							
								...
								executionSaveStatus = executionService.saveExecutionDetails(execName,[scriptName:scriptName, deviceName:deviceName, scriptGroupInstance:scriptGroupInstance,
									appUrl:url, isBenchMark:isBenchMark, isSystemDiagnostics:isSystemDiagnostics, rerun:rerun, isLogReqd:isLogReqd,category:params?.category])
							}
	...
	def getClientPort(final String deviceIP,final String agentPort){
		...
		if(deviceIP && agentPort){
			...
			if(device){
				...
				resultNode.addProperty("logTransferPort",device?.agentMonitorPort?.toString())// here new change 
