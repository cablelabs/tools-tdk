#!/bin/sh
#*****************************************************************************************************
#File			:SuiteExecuter.sh
#Copyright		:TEL Ltd.
#Description	        :This scripts is used to execute the all open source test suites and test apps 
#*****************************************************************************************************
...
echo "        Enter dlna for DLNA test cases-gupnp "
}
...
case $COMPONENT_NAME in
	 ...
	 BIN_PATH=$GSTREAMER_BIN_PATH
         ...
         ;;
     dlna)
	 BIN_PATH=$DLNA_BIN_PATH
         SEARCH_KEY="DlnaChecks:"
         OPTIONS="NULL"
         LOG_PATH=$DLNA_LOG_PATH
         ;;
     gst_plugin_base)
	 ...
	 ;;
     *)
         echo "At this moment qt_non_gfx , qt_gfx, webkit, dlna, gstreamer,gstreamer base plugin and gstreamer good plugin tests can run "
         exit
         ;;
esac
...
#TOTAL_FILE_COUNT=100
	...
	do
      		...
      		if [[ "$COMP_NAME" == "$GRAPHICS" || "$COMP_NAME" == "$WEBKIT" ]]; then	
        		#./$ELEMENT -qws &> $LOG_PATH/log_$ELEMENT
            	./$ELEMENT -xml &> $LOG_PATH/log_$ELEMENT
                pass=`grep -c -w "pass" $LOG_PATH/log_$ELEMENT`
                fail=`grep -c -w "fail" $LOG_PATH/log_$ELEMENT`
                skip=`grep -c -w "skip" $LOG_PATH/log_$ELEMENT`
                total=$(($pass + $fail + $skip))  
                result=`echo "<ResultSummary>"$KEY_WORD$total "Passed:"$pass "Failed:"$fail "Skipped:"$skip"</ResultSummary>"`
                sed -i "s@<\/TestCase>@${result}\n<\/TestCase>@" $LOG_PATH/log_$ELEMENT
                #lineNum=`sed -n "/<?/=" $LOG_PATH/log_$ELEMENT`
                #echo $lineNum
                #echo "**************"
                #if [ $lineNum != 1 ]; then
                #    sed -i "1,$(($lineNum-1))d" $LOG_PATH/log_$ELEMENT
                #    echo "**************"
                #fi
                #endline=`sed -n "/<\/TestCase/=" $LOG_PATH/log_$ELEMENT`
                #var1=`echo $endline`
                #head -$var1 $LOG_PATH/log_$ELEMENT>TEMPFILE
                #cat TEMPFILE>$LOG_PATH/log_$ELEMENT
                #echo "You are login as: `whoami`"
                #echo $ELEMENT
                #echo $LOG_PATH
                #sh $OPENSOURCETEST_PATH/appendtestdetails.sh $ELEMENT $LOG_PATH
      		else
		...
		echo "Exec check: "$EXECUTION_CHECK
